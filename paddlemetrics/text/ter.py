...
class TranslationEditRate(Metric):
    ...
    - ``ter`` (:class:`~paddle.Tensor`): if ``return_sentence_level_score=True`` return a corpus-level translation
    ...
    Example:
        >>> from paddlemetrics.text import TranslationEditRate
    ...
    def compute(self) -> Union[paddle.Tensor, tuple[paddle.Tensor, paddle.Tensor]]:
        ...
        if self.sentence_ter is not None:
            return ter, paddle.concat(self.sentence_ter)
        return ter
