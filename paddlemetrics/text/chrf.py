...
class CHRFScore(Metric):
    ...
    - ``chrf`` (:class:`~paddle.Tensor`): If `return_sentence_level_score=True` return a list of sentence-level
    ...
    Example:
        >>> from paddlemetrics.text import CHRFScore
        ...
        **kwargs: Any,
        ...
        for (n_gram_level, n_gram_order), text in self._get_text_n_gram_iterator():
            for n in range(1, n_gram_order + 1):
                ...
                self.add_state(state_name, paddle.tensor(0.0), dist_reduce_fx="sum")
    ...
    def _convert_states_to_dicts(self) -> _DICT_STATES_TYPES:
        ...
        n_grams_dicts: dict[str, dict[int, paddle.Tensor]] = dict(
            zip(
                _DICT_STATES_NAMES,
                _prepare_n_grams_dicts(self.n_char_order, self.n_word_order),
            )
        )
        for (n_gram_level, n_gram_order), text in self._get_text_n_gram_iterator():
            for n in range(1, n_gram_order + 1):
