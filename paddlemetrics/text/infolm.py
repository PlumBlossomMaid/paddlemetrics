...
from paddlemetrics.functional.text.infolm import (
    _ALLOWED_INFORMATION_MEASURE_LITERAL, _get_dataloader,
    _get_special_tokens_map, _infolm_compute, _infolm_update,
    _InformationMeasure)
...
class InfoLM(Metric):
    ...
    -  ``infolm`` (:class:`~paddle.Tensor`): If `return_sentence_level_score=True` return a tuple with a tensor
    ...
    def __init__(
        ...
        **kwargs: dict[str, Any]
        ...
        self.alpha = alpha
        self.beta = beta
        self._device = paddle.device(device or "cpu")
        self.batch_size = batch_size
        self.num_threads = num_threads
        self.verbose = verbose
        self.return_sentence_level_score = return_sentence_level_score
        self.tokenizer, self.model = _load_tokenizer_and_model(
            model_name_or_path, device
        )
        self.information_measure_cls = _InformationMeasure(
            information_measure, alpha, beta
        )
    ...
    def update(
        ...
        ) = _infolm_update(preds, target, self.tokenizer, self.max_length)
