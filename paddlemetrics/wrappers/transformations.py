...
class MetricInputTransformer(WrapperMetric):
    ...
    def __init__(
        ...
        if not isinstance(wrapped_metric, (Metric, MetricCollection)):
            raise TypeError(
                f"Expected wrapped metric to be an instance of `paddlemetrics.Metric` or `paddlemetrics.MetricsCollection`but received {wrapped_metric}"
            )
    ...
    def _wrap_transform(self, *args: paddle.Tensor) -> tuple[paddle.Tensor, ...]:
...
class LambdaInputTransformer(MetricInputTransformer):
    ...
    Example:
        ...
        >>> from paddlemetrics.wrappers import LambdaInputTransformer
        >>>
    ...
    def __init__(
        ...
        transform_pred: Optional[Callable[[paddle.Tensor], paddle.Tensor]] = None,
        transform_target: Optional[Callable[[paddle.Tensor], paddle.Tensor]] = None,
        **kwargs: Any,
    ) -> None:
