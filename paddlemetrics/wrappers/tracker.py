...
class MetricTracker(paddle.nn.LayerList):
    ...
    Args:
        metric: instance of a ``paddlemetrics.Metric`` or ``paddlemetrics.MetricCollection``
    ...
    Example (multiple metrics using MetricCollection):
        ...
        >>> from paddlemetrics.regression import MeanSquaredError, ExplainedVariance
    ...
    def __init__(
        ...
        maximize: Union[bool, list[bool], None] = None,
    ) -> None:
        super().__init__()
        if not isinstance(metric, (Metric, MetricCollection)):
            raise TypeError(
                f"Metric arg need to be an instance of a paddlemetrics `Metric` or `MetricCollection` but got {metric}"
            )
    ...
    def best_metric(
        ...
        float,
        paddle.Tensor,
        tuple[Union[int, float, paddle.Tensor], Union[int, float, paddle.Tensor]],
        tuple[None, None],
    ...
    def plot(
        ...
        fig, ax = plot_single_or_multi_val(val, ax=ax, name=self.__class__.__name__)
        return fig, ax
