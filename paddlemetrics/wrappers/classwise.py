...
class ClasswiseWrapper(WrapperMetric):
    ...
    Example::
        Classwise can also be used in combination with :class:`~paddlemetrics.MetricCollection`. In this case, everything
        will be flattened into a single dictionary:

        >>> from paddle import randint, randn
        >>> from paddlemetrics import MetricCollection
        >>> from paddlemetrics.wrappers import ClasswiseWrapper
        >>> from paddlemetrics.classification import MulticlassAccuracy, MulticlassRecall
        >>> labels = ["horse", "fish", "dog"]
