...
class ProcrustesDisparity(Metric):
    """Compute the `Procrustes Disparity`_.
    ...
    metric: ``paddlemetrics.functional.shape.procrustes.procrustes_disparity``.

    As input to ``forward`` and ``update`` the metric accepts the following input:

        - ``point_cloud1`` (paddle.Tensor): A tensor of shape ``(N, M, D)`` with ``N`` being the batch size,
          ``M`` the number of data points and ``D`` the dimensionality of the data points.
        - ``point_cloud2`` (paddle.Tensor): A tensor of shape ``(N, M, D)`` with ``N`` being the batch size,
          ``M`` the number of data points and ``D`` the dimensionality of the data points.


    As output to ``forward`` and ``compute`` the metric returns the following output:

        - ``gds`` (:class:`~paddle.Tensor`): A scalar tensor with the Procrustes Disparity.
    ...
    def update(self, point_cloud1: paddle.Tensor, point_cloud2: paddle.Tensor) -> None:
    ...
    def plot(
        ...
        .. plot::
            ...
            >>> from paddlemetrics.shape import ProcrustesDisparity
            >>> metric = ProcrustesDisparity()
            >>> metric.update(paddle.randn(10, 50, 2), paddle.randn(10, 50, 2))
        ...
        .. plot::
            ...
            >>> values = [ ]
            >>> for _ in range(10):
            ...     values.append(metric(paddle.randn(10, 50, 2), paddle.randn(10, 50, 2)))
