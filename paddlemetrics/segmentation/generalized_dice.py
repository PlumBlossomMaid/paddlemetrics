...
class GeneralizedDiceScore(Metric):
    ...
    As output to ``forward`` and ``compute`` the metric returns the following output:
        ...
        - ``gds`` (:class:`~paddle.Tensor`): The generalized dice score. If ``per_class`` is set to ``True``, the output
    ...
    Example:
        ...
        >>> from paddlemetrics.segmentation import GeneralizedDiceScore
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        self.add_state("samples", default=paddle.zeros(1), dist_reduce_fx="sum")
