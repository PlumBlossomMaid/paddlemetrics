...
class MeanIoU(Metric):
    ...
    As output to ``forward`` and ``compute`` the metric returns the following output:
        ...
        - ``miou`` (:class:`~paddle.Tensor`): The mean Intersection over Union (mIoU) score. If ``per_class`` is set to
    ...
    Example:
        ...
        >>> from paddlemetrics.segmentation import MeanIoU
        >>> miou = MeanIoU()
        >>> preds = randint(0, 2, (10, 3, 128, 128), generator=paddle.Generator().manual_seed(42))
        >>> target = randint(0, 2, (10, 3, 128, 128), generator=paddle.Generator().manual_seed(43))
        >>> miou(preds, target)
        tensor(0.3336)
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if num_classes is not None:
            ...
            self.add_state("num_batches", default=paddle.zeros(1), dist_reduce_fx="sum")
