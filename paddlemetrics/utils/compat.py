...
def pack_padded_sequence(
    input: paddle.Tensor, lengths: paddle.Tensor, batch_first: bool = False, enforce_sorted: bool = True
    ...
    # 生成逆序索引（用于解包时恢复原顺序）
    unsorted_indices = (
        paddle.argsort(sorted_indices) if not enforce_sorted else paddle.arange(batch_size, dtype=paddle.int64)
    )
