...
def _generate_wave_table(
    ...
    if wave_type == "SINE":
        d = (paddle.sin(point.to(paddle.float64) / table_size * 2 * math.pi) + 1) / 2
...
def band_biquad(
    ...
    if noise:
        mult = paddle.sqrt(((1 + a2) * (1 + a2) - a1 * a1) * (1 - a2) / (1 + a2)) / b0
        b0 = mult * b0
    b1 = 0.0
    b2 = 0.0
    return biquad(waveform, b0, b1, b2, a0, a1, a2)
...
def vad(
    ...
    cepstrum_window *= paddle.audio.functional.get_window(
        win_length=spectrum_end - spectrum_start, dtype=paddle.float32, window="hann"
    )
