...
class VideoMultiMethodAssessmentFusion(Metric):
    ...
    .. note::
       ...
       or with ``pip install paddlemetrics[video]``.

    As input to ``forward`` and ``update`` the metric accepts the following input:

        - ``preds`` (:class:`~paddle.Tensor`): Video tensor of shape ``(batch, channels, frames, height, width)``.
          Expected to be in RGB format with values in range [0, 1].
        - ``target`` (:class:`~paddle.Tensor`): Video tensor of shape ``(batch, channels, frames, height, width)``.
          Expected to be in RGB format with values in range [0, 1].

    As output of ``forward`` and ``compute`` the metric returns the following output ``vmaf`` (:class:`~paddle.Tensor`):
    ...
    Example:
        ...
        >>> paddle.round(vmaf(preds, target), decimals=2)
    ...
    vmaf_score: List[paddle.Tensor]
    integer_motion2: List[paddle.Tensor]
    integer_motion: List[paddle.Tensor]
    integer_adm2: List[paddle.Tensor]
    integer_adm_scale0: List[paddle.Tensor]
    integer_adm_scale1: List[paddle.Tensor]
    integer_adm_scale2: List[paddle.Tensor]
    integer_adm_scale3: List[paddle.Tensor]
    integer_vif_scale0: List[paddle.Tensor]
    integer_vif_scale1: List[paddle.Tensor]
    integer_vif_scale2: List[paddle.Tensor]
    integer_vif_scale3: List[paddle.Tensor]
    ...
    def __init__(self, features: bool = False, **kwargs: Any) -> None:
        ...
        if not isinstance(features, bool):
            raise ValueError(
                "Argument `elementary_features` should be a boolean, but got {features}."
            )
    ...
    def update(self, preds: paddle.Tensor, target: paddle.Tensor) -> None:
        ...
        elif isinstance(score, paddle.Tensor):
            self.vmaf_score.append(score)

    def compute(self) -> Union[paddle.Tensor, Dict[str, paddle.Tensor]]:
