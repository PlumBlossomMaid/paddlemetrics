...
def _fleiss_kappa_update(
    ...
    if mode == "probs":
        ...
        raise ValueError(
            "If argument ``mode`` is `counts`, ratings must have 2 dimensions with the format [n_samples, n_categories] and be none floating point."
        )
    return ratings


def _fleiss_kappa_compute(counts: paddle.Tensor) -> paddle.Tensor:
...
def fleiss_kappa(
    ...
    Example:
        ...
        >>> from paddlemetrics.functional.nominal import fleiss_kappa
