...
from paddlemetrics.functional.clustering.mutual_info_score import _mutual_info_score_compute, _mutual_info_score_update
...
def expected_mutual_info_score(contingency: paddle.Tensor, n_samples: int) -> paddle.Tensor:
    ...
    nijs = paddle.arange(0, max([a.amax().item(), b.amax().item()]) + 1, device=a.device)
    nijs[0] = 1
    ...
    gln_nnij = paddle.lgamma(x=nijs + 1) + paddle.lgamma(x=paddle.tensor(n_samples + 1, dtype=a.dtype, device=a.place))
