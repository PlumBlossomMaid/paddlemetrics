...
def retrieval_average_precision(
    ...
    positions = paddle.arange(1, len(target) + 1, device=target.device, dtype=paddle.float32)[target > 0]
    return paddle.div(
        paddle.arange(len(positions), device=positions.device, dtype=paddle.float32) + 1,
        positions,
    ).mean()
