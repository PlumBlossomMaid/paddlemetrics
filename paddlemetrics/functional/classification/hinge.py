...
def binary_hinge_loss(
    ...
    Example:
        ...
        >>> from paddlemetrics.functional.classification import binary_hinge_loss
...
def _multiclass_hinge_loss_update(
    ...
    if multiclass_mode == "crammer-singer":
        margin = preds[target]
        margin -= paddle.max(preds[~target].reshape(preds.shape[0], -1), axis=1)[0]
    else:
        target = target.bool()
        margin = paddle.zeros_like(preds)
        margin[target] = preds[target]
...
def multiclass_hinge_loss(
    ...
    Example:
        >>> from paddle import tensor
        >>> from paddlemetrics.functional.classification import multiclass_hinge_loss
...
def hinge_loss(
    ...
    :func:`~paddlemetrics.functional.classification.binary_hinge_loss` and
    :func:`~paddlemetrics.functional.classification.multiclass_hinge_loss` for the specific details of
