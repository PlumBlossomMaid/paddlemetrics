...
def _iou_compute(iou: paddle.Tensor, aggregate: bool = True) -> paddle.Tensor:
    if not aggregate:
        return iou
...
def intersection_over_union(
    ...
    Example::
        ...
        >>> import paddle
        >>> from paddlemetrics.functional.detection import intersection_over_union
    ...
    if not _TORCHVISION_AVAILABLE:
        raise ModuleNotFoundError(
            f"`{intersection_over_union.__name__}` requires that `torchvision` is installed. Please install with `pip install paddlemetrics[detection]`."
        )
