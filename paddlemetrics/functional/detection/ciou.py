...
def _ciou_compute(iou: paddle.Tensor, aggregate: bool = True) -> paddle.Tensor:
    if not aggregate:
        return iou
...
def complete_intersection_over_union(
    ...
    """Compute Complete Intersection over Union (`CIOU`_) between two sets of boxes.
    ...
    Example::
        ...
        >>> import paddle
        >>> from paddlemetrics.functional.detection import complete_intersection_over_union
    ...
    if not _TORCHVISION_AVAILABLE:
        raise ModuleNotFoundError(
            f"`{complete_intersection_over_union.__name__}` requires that `torchvision` is installed. Please install with `pip install paddlemetrics[detection]`."
        )
