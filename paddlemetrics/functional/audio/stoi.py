...
from paddlemetrics.utils.imports import _PYSTOI_AVAILABLE
...
def short_time_objective_intelligibility(
    ...
    Example:
        ...
        >>> preds = randn([8000])
        >>> target = randn([8000])
        >>> short_time_objective_intelligibility(preds, target, 8000).astype(paddle.float32)
        tensor(-0.084...)

    """
    if not _PYSTOI_AVAILABLE:
        raise ModuleNotFoundError(
            "ShortTimeObjectiveIntelligibility metric requires that `pystoi` is installed. Either install as `pip install paddlemetrics[audio]` or `pip install pystoi`."
        )
