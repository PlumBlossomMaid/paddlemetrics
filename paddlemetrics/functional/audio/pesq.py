...
def perceptual_evaluation_speech_quality(
    ...
    .. hint::
        ...
        paddlemetrics[audio]`` or ``pip install pesq``. Note that ``pesq`` will compile with your currently
    ...
    if not _PESQ_AVAILABLE:
        raise ModuleNotFoundError(
            "PESQ metric requires that pesq is installed. Either install as `pip install paddlemetrics[audio]` or `pip install pesq`."
        )
