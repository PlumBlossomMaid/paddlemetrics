...
__doctest_requires__ = {
    ("deep_noise_suppression_mean_opinion_score", "_load_session"): [
        "requests",
        "librosa",
        "onnxruntime",
    ]
...
SAMPLING_RATE = 16000
INPUT_LENGTH = 9.01
DNSMOS_DIR = "~/.paddlemetrics/DNSMOS"
...
def _prepare_dnsmos(dnsmos_dir: str) -> None:
    ...
    Args:
        dnsmos_dir: a dir to save the downloaded files. Defaults to "~/.paddlemetrics".
...
def deep_noise_suppression_mean_opinion_score(
    ...
    .. hint::
        ...
        as ``pip install paddlemetrics['audio']`` or alternatively ``pip install librosa onnxruntime-gpu requests``
    ...
    device = paddle.device(device) if device is not None else preds.place
    ...
    for idx in range(num_hops):
        ...
        moss.append(mos_np)
    return paddle.to_tensor(np.mean(np.stack(moss, axis=-1), axis=-1))