...
from paddlemetrics.functional.text.infolm import \
    _ALLOWED_INFORMATION_MEASURE_LITERAL as \
    _INFOLM_ALLOWED_INFORMATION_MEASURE_LITERAL
...
def _bert_score(
    preds: Union[list[str], dict[str, paddle.Tensor]],
    target: Union[list[str], dict[str, paddle.Tensor]],
...
) -> dict[str, Union[paddle.Tensor, list[float], str]]:
...
def _bleu_score(
    ...
    return bleu_score(
        preds=preds, target=target, n_gram=n_gram, smooth=smooth, weights=weights
    )
...
def _squad(
    preds: Union[dict[str, str], list[dict[str, str]]], target: SQUAD_TARGETS_TYPE
) -> dict[str, paddle.Tensor]:
    """Wrapper for deprecated import.
...
def _word_information_preserved(
    preds: Union[str, list[str]], target: Union[str, list[str]]
) -> paddle.Tensor:
    """Wrapper for deprecated import.
