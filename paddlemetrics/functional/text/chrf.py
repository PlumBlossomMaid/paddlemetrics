...
_EPS_SMOOTHING = paddle.tensor(1e-16)
_PUNCTUATIONS = set("!\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~")
...
def _ngram_counts(
    ...
    """
    ngrams: dict[int, dict[tuple[str, ...], paddle.Tensor]] = defaultdict(
...
def _get_ngram_matches(
    hyp_n_grams_counts: dict[int, dict[tuple[str, ...], paddle.Tensor]],
    ref_n_grams_counts: dict[int, dict[tuple[str, ...], paddle.Tensor]],
) -> dict[int, paddle.Tensor]:
...
def _sum_over_dicts(
    total_n_grams: dict[int, paddle.Tensor], n_grams: dict[int, paddle.Tensor]
) -> dict[int, paddle.Tensor]:
...
def _calculate_fscore(
    matching_char_n_grams: dict[int, paddle.Tensor],
    matching_word_n_grams: dict[int, paddle.Tensor],
    hyp_char_n_grams: dict[int, paddle.Tensor],
    hyp_word_n_grams: dict[int, paddle.Tensor],
    ref_char_n_grams: dict[int, paddle.Tensor],
    ref_word_n_grams: dict[int, paddle.Tensor],
    n_order: float,
    beta: float,
) -> paddle.Tensor:
    ...
    def _get_n_gram_fscore(
        matching_n_grams: dict[int, paddle.Tensor],
        ref_n_grams: dict[int, paddle.Tensor],
        hyp_n_grams: dict[int, paddle.Tensor],
        beta: float,
    ) -> dict[int, paddle.Tensor]:
...
def _calculate_sentence_level_chrf_score(
    targets: list[str],
    pred_char_n_grams_counts: dict[int, dict[tuple[str, ...], paddle.Tensor]],
    pred_word_n_grams_counts: dict[int, dict[tuple[str, ...], paddle.Tensor]],
    pred_char_n_grams: dict[int, paddle.Tensor],
    pred_word_n_grams: dict[int, paddle.Tensor],
    n_char_order: int,
    n_word_order: int,
    n_order: float,
    beta: float,
...
def _chrf_score_update(
    ...
    lowercase: bool,
    whitespace: bool,
    sentence_chrf_score: Optional[List[paddle.Tensor]] = None,
) -> tuple[
    dict[int, paddle.Tensor],
    dict[int, paddle.Tensor],
    dict[int, paddle.Tensor],
...
def chrf_score(
    ...
    Example:
        >>> from paddlemetrics.functional.text import chrf_score
    ...
    if sentence_chrf_score:
        return chrf_f_score, paddle.concat(sentence_chrf_score)
    return chrf_f_score
