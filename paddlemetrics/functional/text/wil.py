...
def _word_info_lost_update(
    ...
    if isinstance(target, str):
        target = [target]
    total = paddle.tensor(0.0)
    errors = paddle.tensor(0.0)
    target_total = paddle.tensor(0.0)
    preds_total = paddle.tensor(0.0)
...
def word_information_lost(
    ...
    Examples:
        >>> from paddlemetrics.functional.text import word_information_lost
