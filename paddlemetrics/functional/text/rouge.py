...
def _ensure_nltk_punkt_is_downloaded() -> None:
    ...
    try:
        ...
        try:
            ...
            raise OSError(
                "`nltk` resource `punkt` is not available on a disk and cannot be downloaded as a machine is not connected to the internet."
            ) from err
...
def _compute_metrics(
    ...
    if precision == recall == 0.0:
        return {
            "precision": paddle.tensor(0.0),
            "recall": paddle.tensor(0.0),
            "fmeasure": paddle.tensor(0.0),
        }
    fmeasure = 2 * precision * recall / (precision + recall)
    return {
        "precision": paddle.tensor(precision),
        "recall": paddle.tensor(recall),
        "fmeasure": paddle.tensor(fmeasure),
    }
...
def _rouge_score_update(
    ...
    normalizer: Optional[Callable[[str], str]] = None,
    tokenizer: Optional[Callable[[str], Sequence[str]]] = None,
) -> dict[Union[int, str], list[dict[str, paddle.Tensor]]]:
    ...
    for pred_raw, target_raw in zip(preds, target):
        ...
        result_avg: dict[Union[int, str], list[dict[str, paddle.Tensor]]] = {
            rouge_key: [] for rouge_key in rouge_keys_values
        }
        list_results = []
        ...
        if accumulate == "best":
            ...
            for rouge_key, metrics in result_avg.items():
                _dict_metric_score_batch: dict[str, List[paddle.Tensor]] = {}
...
def _rouge_score_compute(
    ...
    """
    results: dict[str, paddle.Tensor] = {}
    if sentence_results == {}:
        return results
    for rouge_key, scores in sentence_results.items():
        results[rouge_key] = paddle.tensor(scores).mean()
    return results
