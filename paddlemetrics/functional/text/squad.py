...
def _compute_f1_score(predicted_answer: str, target_answer: str) -> paddle.Tensor:
    ...
    num_same = paddle.tensor(sum(common.values()))
    if len(target_tokens) == 0 or len(predicted_tokens) == 0:
        return paddle.tensor(int(target_tokens == predicted_tokens))
    if num_same == 0:
        return paddle.tensor(0.0)
    precision = 1.0 * num_same / paddle.tensor(len(predicted_tokens))
    recall = 1.0 * num_same / paddle.tensor(len(target_tokens))
...
def _metric_max_over_ground_truths(
    metric_fn: Callable[[str, str], paddle.Tensor],
    prediction: str,
...
def _squad_input_check(
    ...
    for pred in preds:
        ...
        if "prediction_text" not in pred_keys or "id" not in pred_keys:
            raise KeyError(
                "Expected keys in a single prediction are 'prediction_text' and 'id'.Please make sure that 'prediction_text' maps to the answer string and 'id' maps to the key string."
            )
    for target in targets:
        target_keys = target.keys()
        if "answers" not in target_keys or "id" not in target_keys:
            raise KeyError(
                f"""Expected keys in a single target are 'answers' and 'id'.Please make sure that 'answers' maps to a `SQuAD` format dictionary and 'id' maps to the key string.
...
def _squad_update(
    ...
    Example:
        >>> from paddlemetrics.functional.text.squad import _squad_update
    ...
    """
    f1 = paddle.tensor(0.0)
    exact_match = paddle.tensor(0.0)
    total = paddle.tensor(0)
...
def squad(preds: PREDS_TYPE, target: TARGETS_TYPE) -> dict[str, paddle.Tensor]:
