...
def _process_attention_mask_for_special_tokens(
    ...
    """
    attention_mask[:, 0] = 0
    sep_token_position = _cumsum(attention_mask - 0.1, axis=-1).argmax(-1)
    attention_mask[paddle.arange(attention_mask.size(0)).long(), sep_token_position] = 0
    return attention_mask


def _input_data_collator(
    batch: dict[str, paddle.Tensor], device: Optional[Union[str, paddle.device]] = None
    ...
    """
    max_len = int(batch["attention_mask"].sum(1)._max().item())
...
def _output_data_collator(
    model_output: paddle.Tensor, attention_mask: paddle.Tensor, target_len: int
    ...
    model_output = paddle.concat(
        [
            model_output,
            paddle.zeros(zeros_shape, dtype=model_output.dtype).to(model_output.place),
        ], axis=2,
    )
    zeros = paddle.zeros(zeros_shape[0], zeros_shape[2], dtype=attention_mask.dtype).to(
        attention_mask.device
    )
    attention_mask = paddle.concat([attention_mask, zeros], axis=1)
    return model_output, attention_mask


def _sort_data_according_length(
    input_ids: paddle.Tensor, attention_mask: paddle.Tensor
...
) -> tuple[dict[str, paddle.Tensor], Optional[paddle.Tensor]]:
...
def _get_progress_bar(
    dataloader: paddle.io.DataLoader, verbose: bool = False
) -> Union[paddle.io.DataLoader, "tqdm.auto.tqdm"]:
...
def _check_shape_of_model_output(
    output: paddle.Tensor, input_ids: paddle.Tensor
) -> None:
    """Check if the shape of the user's own model output."""
    bs, seq_len = input_ids.shape[:2]
    invalid_out_shape = (
        len(output.shape) != 3 or output.shape[0] != bs or output.shape[1] != seq_len
    )
    if invalid_out_shape:
        raise ValueError(
            f"The model output must be `Tensor` of a shape `[batch_size, seq_len, model_dim]` i.e. [{bs}, {seq_len}. , `model_dim`], but got {output.shape}."
        )
...
class TextDataset(paddle.io.Dataset):
    ...
    def __init__(
        ...
        if idf:
            self.tokens_idf = (
                tokens_idf if tokens_idf is not None else self._get_tokens_idf()
            )
    ...
    def __getitem__(self, idx: int) -> dict[str, paddle.Tensor]:
        ...
        if self.idf:
            input_ids_idf = paddle.tensor(
                [self.tokens_idf[input_idx] for input_idx in input_ids.tolist()]
            )
    ...
    @staticmethod
    def _set_of_tokens(input_ids: paddle.Tensor) -> set:
        """Return set of tokens from the `input_ids` :class:`~paddle.Tensor`."""
