...
def _ssim_check_inputs(
    ...
    if len(preds.shape) not in (4, 5):
        raise ValueError(
            f"Expected `preds` and `target` to have BxCxHxW or BxCxDxHxW shape. Got preds: {preds.shape} and target: {target.shape}."
        )
    return preds, target
...
def structural_similarity_index_measure(
    ...
    Example:
        >>> from paddlemetrics.functional.image import structural_similarity_index_measure
...
def _get_normalized_sim_and_cs(
    ...
    if normalize == "relu":
        sim = paddle.nn.functional.relu(x=sim)
        contrast_sensitivity = paddle.nn.functional.relu(x=contrast_sensitivity)
...
def _multiscale_ssim_update(
    ...
    if preds.size()[-1] < 2 ** len(betas) or preds.size()[-2] < 2 ** len(betas):
        raise ValueError(
            f"For a given number of `betas` parameters {len(betas)}, the image height and width dimensions must be larger than or equal to {2 ** len(betas)}."
        )
    _betas_div = max(1, len(betas) - 1) ** 2
    if preds.size()[-2] // _betas_div <= kernel_size[0] - 1:
        raise ValueError(
            f"For a given number of `betas` parameters {len(betas)} and kernel size {kernel_size[0]}, the image height must be larger than {(kernel_size[0] - 1) * _betas_div}."
        )
    if preds.size()[-1] // _betas_div <= kernel_size[1] - 1:
        raise ValueError(
            f"For a given number of `betas` parameters {len(betas)} and kernel size {kernel_size[1]}, the image width must be larger than {(kernel_size[1] - 1) * _betas_div}."
        )
...
def multiscale_structural_similarity_index_measure(
    ...
    Example:
        ...
        >>> from paddlemetrics.functional.image import multiscale_structural_similarity_index_measure
