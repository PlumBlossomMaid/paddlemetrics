...
def _gaussian(
    ...
    sigma: float,
    dtype: paddle.dtype,
    device: Union[paddle.device, str],
) -> paddle.Tensor:
    ...
    Example:
        >>> _gaussian(3, 1, paddle.float, 'cpu')
    ...
    gauss = paddle.exp(-paddle.pow(dist / sigma, 2) / 2)
...
def _gaussian_kernel_2d(
    ...
    Example:
        >>> _gaussian_kernel_2d(1, (5,5), (1,1), paddle.float, "cpu")
    ...
    kernel = paddle.matmul(gaussian_kernel_x.t(), gaussian_kernel_y)
...
def _single_dimension_pad(
    ...
    y = paddle.index_select(
        inputs,
        dim,
        paddle.arange(_max - 1, _max - pad - outer_pad, -1).to(inputs.place),
    )
...
def _uniform_filter(inputs: paddle.Tensor, window_size: int) -> paddle.Tensor:
...
def _gaussian_kernel_3d(
    ...
    kernel_xy = paddle.matmul(gaussian_kernel_x.t(), gaussian_kernel_y)
    kernel = paddle.mul(
