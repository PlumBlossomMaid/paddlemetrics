...
class GeneratorType(paddle.nn.Layer):
    ...
    def sample(self, num_samples: int) -> paddle.Tensor:
...
def _validate_generator_model(
    generator: GeneratorType, conditional: bool = False
) -> None:
    ...
    if not hasattr(generator, "sample"):
        raise NotImplementedError(
            "The generator must have a `sample` method with signature `sample(num_samples: int) -> Tensor` where the returned tensor has shape `(num_samples, z_size)`."
        )
...
def _perceptual_path_length_validate_arguments(
    ...
    if not (isinstance(num_samples, int) and num_samples > 0):
        raise ValueError(
            f"Argument `num_samples` must be a positive integer, but got {num_samples}."
        )
    if not isinstance(conditional, bool):
        raise ValueError(
            f"Argument `conditional` must be a boolean, but got {conditional}."
        )
    if not (isinstance(batch_size, int) and batch_size > 0):
        raise ValueError(
            f"Argument `batch_size` must be a positive integer, but got {batch_size}."
        )
    ...
    if not (isinstance(epsilon, float) and epsilon > 0):
        raise ValueError(
            f"Argument `epsilon` must be a positive float, but got {epsilon}."
        )
    if resize is not None and not (isinstance(resize, int) and resize > 0):
        raise ValueError(
            f"Argument `resize` must be a positive integer or `None`, but got {resize}."
        )
    if lower_discard is not None and not (
        isinstance(lower_discard, float) and 0 <= lower_discard <= 1
    ):
        raise ValueError(
            f"Argument `lower_discard` must be a float between 0 and 1 or `None`, but got {lower_discard}."
        )
    if upper_discard is not None and not (
        isinstance(upper_discard, float) and 0 <= upper_discard <= 1
    ):
        raise ValueError(
...
def _interpolate(
    latents1: paddle.Tensor,
    latents2: paddle.Tensor,
    epsilon: float = 0.0001,
    interpolation_method: Literal["lerp", "slerp_any", "slerp_unit"] = "lerp",
) -> paddle.Tensor:
    ...
    if interpolation_method == "slerp_any":
        ...
        latents1_norm = latents1 / (latents1**2).sum(
            dim=-1, keepdim=True
        ).sqrt().clamp_min(eps)
        latents2_norm = latents2 / (latents2**2).sum(
            dim=-1, keepdim=True
        ).sqrt().clamp_min(eps)
        d = (latents1_norm * latents2_norm).sum(dim=-1, keepdim=True)
        mask_zero = (latents1_norm.norm(dim=-1, keepdim=True) < eps) | (
            latents2_norm.norm(dim=-1, keepdim=True) < eps
        )
        mask_collinear = (d > 1 - eps) | (d < -1 + eps)
        mask_lerp = (mask_zero | mask_collinear).repeat(
            [(1) for _ in range(ndims)] + [z_size]
        )
        omega = d.acos()
        ...
        out[mask_lerp] = _interpolate(
            latents1, latents2, epsilon, interpolation_method="lerp"
        )[mask_lerp]
        return out
    if interpolation_method == "slerp_unit":
        out = _interpolate(
            ...
            latents2=latents2,
            epsilon=epsilon,
            interpolation_method="slerp_any",
        )
...
def perceptual_path_length(
    ...
    lower_discard: Optional[float] = 0.01,
    upper_discard: Optional[float] = 0.99,
    sim_net: Union[paddle.nn.Layer, Literal["alex", "vgg", "squeeze"]] = "vgg",
    ...
    Args:
        ...
        sim_net: Similarity network to use. Can be a `nn.Layer` or one of 'alex', 'vgg', 'squeeze', where the three
    ...
    Example::
        >>> import paddle
        >>> from paddlemetrics.functional.image import perceptual_path_length
        >>> class DummyGenerator(paddle.nn.Layer):
        ...    def __init__(self, z_size) -> None:
        ...       super().__init__()
        ...       self.z_size = z_size
        ...       self.model = paddle.nn.Sequential(paddle.nn.Linear(z_size, 3*128*128), paddle.nn.Sigmoid())
        ...    def forward(self, z):
    ...
    if not _TORCHVISION_AVAILABLE:
        raise ModuleNotFoundError(
            "Metric `perceptual_path_length` requires torchvision which is not installed.Install with `pip install torchvision` or `pip install paddlemetrics[image]`"
        )
    ...
    latent1 = generator.sample(shape=num_samples).to(device)
    latent2 = generator.sample(shape=num_samples).to(device)
    latent2 = _interpolate(
        latent1, latent2, epsilon, interpolation_method=interpolation_method
    )
    if conditional:
        labels = paddle.randint(
            low=0, high=generator.num_classes, shape=(num_samples,)
        ).to(device)
    if isinstance(sim_net, paddle.nn.Layer):
        net = sim_net.to(device)
    elif sim_net in ["alex", "vgg", "squeeze"]:
        net = _LPIPS(pretrained=True, net=sim_net, resize=resize).to(device)
    else:
        raise ValueError(
            f"sim_net must be a nn.Layer or one of 'alex', 'vgg', 'squeeze', got {sim_net}"
        )
    with paddle.no_grad():
        distances = []
        num_batches = math.ceil(num_samples / batch_size)
        for batch_idx in range(num_batches):
            batch_latent1 = latent1[
                batch_idx * batch_size : (batch_idx + 1) * batch_size
            ].to(device)
            batch_latent2 = latent2[
                batch_idx * batch_size : (batch_idx + 1) * batch_size
            ].to(device)
            if conditional:
                batch_labels = labels[
                ...
                outputs = generator(
                    paddle.concat((batch_latent1, batch_latent2), axis=0),
                    paddle.concat((batch_labels, batch_labels), axis=0),
                )
            else:
                outputs = generator(paddle.concat((batch_latent1, batch_latent2), axis=0))
            out1, out2 = outputs.chunk(2, axis=0)
        ...
        distances = paddle.concat(distances)
        lower = (
            paddle.quantile(distances, lower_discard, interpolation="lower")
            if lower_discard is not None
            else 0.0
        )
        upper = (
            paddle.quantile(distances, upper_discard, interpolation="lower")
