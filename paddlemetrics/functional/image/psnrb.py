...
def _compute_bef(x: paddle.Tensor, block_size: int = 8) -> paddle.Tensor:
    ...
    h_b = paddle.to_tensor(list(range(block_size - 1, width - 1, block_size)))
    h_bc = paddle.to_tensor(list(set(h.tolist()).symmetric_difference(h_b.tolist())))
    v = paddle.arange(height - 1)
    v_b = paddle.to_tensor(list(range(block_size - 1, height - 1, block_size)))
    v_bc = paddle.to_tensor(list(set(v.tolist()).symmetric_difference(v_b.tolist())))
