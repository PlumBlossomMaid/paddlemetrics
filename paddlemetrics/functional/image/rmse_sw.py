...
def _rmse_sw_update(
    ...
    window_size: int,
    rmse_val_sum: Optional[paddle.Tensor],
    rmse_map: Optional[paddle.Tensor],
    total_images: Optional[paddle.Tensor],
    ...
    if total_images is not None:
        ...
        total_images = paddle.tensor(target.shape[0], device=target.place)
    error = (target - preds) ** 2
    error = _uniform_filter(error, window_size)
    _rmse_map = paddle.sqrt(error)
...
) -> tuple[Optional[paddle.Tensor], paddle.Tensor]:
...
def root_mean_squared_error_using_sliding_window(
    ...
    Example:
        ...
        >>> from paddlemetrics.functional.image import root_mean_squared_error_using_sliding_window
