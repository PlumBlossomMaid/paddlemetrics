...
from paddlemetrics.functional.multimodal.clip_iqa import (
    _clip_iqa_compute, _clip_iqa_format_prompts, _clip_iqa_get_anchor_vectors,
    _clip_iqa_update, _get_clip_iqa_model_and_processor)
...
class CLIPImageQualityAssessment(Metric):
    ...
    - ``images`` (:class:`~paddle.Tensor`): tensor with images feed to the feature extractor with shape ``(N,C,H,W)``

    As output of `forward` and `compute` the metric returns the following output

    - ``clip_iqa`` (:class:`~paddle.Tensor` or dict of tensors): tensor with the CLIP-IQA score. If a single prompt is
    ...
    .. hint::
        ...
        `pip install piq` or `pip install paddlemetrics[image]`.
    ...
    Example::
        ...
        >>> from paddlemetrics.multimodal import CLIPImageQualityAssessment
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        self.model, self.processor = _get_clip_iqa_model_and_processor(
            model_name_or_path
        )
    ...
    def update(self, images: paddle.Tensor) -> None:
        ...
        with paddle.no_grad():
            img_features = _clip_iqa_update(
                ...
                self.data_range,
                self.device)
            probs = _clip_iqa_compute(
                img_features, self.anchors, self.prompts_name, format_as_dict=False
            )
            if not isinstance(probs, paddle.Tensor):
    ...
    def plot(
        ...
        .. plot::
            ...
            >>> metric = CLIPImageQualityAssessment()
            >>> metric.update(paddle.rand(1, 3, 224, 224))
        ...
        .. plot::
            ...
            >>> values = [ ]
            >>> for _ in range(10):
            ...     values.append(metric(paddle.rand(1, 3, 224, 224)))
