...
class LipVertexError(Metric):
    ...
    - ``preds`` (:class:`~paddle.Tensor`): Predicted vertices tensor of shape (T, V, 3) where T is number of frames,
                V is number of vertices, and 3 represents XYZ coordinates
    - ``target`` (:class:`~paddle.Tensor`): Ground truth vertices tensor of shape (T', V, 3) where T' can be different
                from T

    As output of ``forward`` and ``compute``, the metric returns the following output:

    - ``lve_score`` (:class:`~paddle.Tensor`): A scalar tensor containing the mean Lip Vertex Error value across
                all frames.
    ...
    def update(self, vertices_pred: paddle.Tensor, vertices_gt: paddle.Tensor) -> None:
        ...
        if self.validate_args:
            ...
            if max(self.mouth_map) >= vertices_pred.shape[1]:
                raise ValueError(
                    f"mouth_map contains invalid vertex indices. Max index {max(self.mouth_map)} is larger than number of vertices {vertices_pred.shape[1]}."
                )
    ...
    def compute(self) -> paddle.Tensor:
        ...
        Returns:
            paddle.Tensor: A scalar tensor with the mean LVE value
    ...
    def plot(
        ...
        .. plot::
            ...
            >>> values = []
            >>> for _ in range(10):
            ...     vertices_pred = paddle.randn(10, 100, 3, generator=paddle.seed(42+_))
            ...     vertices_gt = paddle.randn(10, 100, 3, generator=paddle.seed(43+_))
