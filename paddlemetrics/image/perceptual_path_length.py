...
from paddlemetrics.functional.image.perceptual_path_length import (
    GeneratorType, _perceptual_path_length_validate_arguments,
    _validate_generator_model, perceptual_path_length)
...
class PerceptualPathLength(Metric):
    ...
    .. hint::
        ...
        Either install as ``pip install paddlemetrics[image]`` or ``pip install torchvision``

    As input to ``forward`` and ``update`` the metric accepts the following input

    - ``generator`` (:class:`~paddle.nn.Layer`):  Generator model, with specific requirements. See above.

    As output of `forward` and `compute` the metric returns the following output

    - ``ppl_mean`` (:class:`~paddle.Tensor`): float scalar tensor with mean PPL value over distances
    - ``ppl_std`` (:class:`~paddle.Tensor`): float scalar tensor with std PPL value over distances
    - ``ppl_raw`` (:class:`~paddle.Tensor`): float scalar tensor with raw PPL distances
        ...
        **kwargs: Any,
        ...
        if not _TORCHVISION_AVAILABLE:
            raise ModuleNotFoundError(
                "Metric `PerceptualPathLength` requires torchvision which is not installed.Install with `pip install torchvision` or `pip install paddlemetrics[image]`"
            )
