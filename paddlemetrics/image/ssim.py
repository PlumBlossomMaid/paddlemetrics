...
if not _MATPLOTLIB_AVAILABLE:
    __doctest_skip__ = [
        "StructuralSimilarityIndexMeasure.plot",
        "MultiScaleStructuralSimilarityIndexMeasure.plot",
    ]
...
class StructuralSimilarityIndexMeasure(Metric):
    ...
    - ``ssim`` (:class:`~paddle.Tensor`): if ``reduction!='none'`` returns float scalar tensor with average SSIM value
...
class MultiScaleStructuralSimilarityIndexMeasure(Metric):
    ...
    - ``msssim`` (:class:`~paddle.Tensor`): if ``reduction!='none'`` returns float scalar tensor with average MSSSIM
    ...
    Example:
        ...
        >>> from paddlemetrics.image import MultiScaleStructuralSimilarityIndexMeasure
        >>> preds = paddle.rand([3, 3, 256, 256])
        >>> target = preds * 0.75
