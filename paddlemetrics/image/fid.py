...
if _TORCH_FIDELITY_AVAILABLE:
    ...
    class _FeatureExtractorInceptionV3(paddle.nn.Layer):
        pass
...
class NoTrainInceptionV3(_FeatureExtractorInceptionV3):
    ...
    def __init__(
        ...
        if not _TORCH_FIDELITY_AVAILABLE:
            raise ModuleNotFoundError(
                "NoTrainInceptionV3 module requires that `Torch-fidelity` is installed. Either install as `pip install paddlemetrics[image]` or `pip install torch-fidelity`."
            )
    ...
    def _torch_fidelity_forward(self, x: paddle.Tensor) -> tuple[paddle.Tensor, ...]:
        ...
        """
        vassert(
            paddle.is_tensor(x) and x.dtype == paddle.uint8,
            "Expecting image as paddle.Tensor with dtype=paddle.uint8",
        )
        features = {}
        remaining_features = self.features_list.copy()
        x = x.to(self._dtype) if hasattr(self, "_dtype") else x.to(paddle.float32)
        ...
        if "64" in remaining_features:
            features["64"] = (
                paddle.nn.functional.adaptive_avg_pool2d(x, output_size=(1, 1))
                .squeeze(-1)
                .squeeze(-1)
            )
...
def _compute_fid(
    mu1: paddle.Tensor, sigma1: paddle.Tensor, mu2: paddle.Tensor, sigma2: paddle.Tensor
) -> paddle.Tensor:
    ...
    c = paddle.linalg.eigvals(x=sigma1 @ sigma2).sqrt().real().sum(dim=-1)
    return a + b - 2 * c


class FrechetInceptionDistance(Metric):
    """Calculate Fréchet inception distance (FID_) which is used to assess the quality of generated images.
    ...
    Using custom feature extractor is also possible. One can give a paddle.nn.Layer as `feature` argument. This
    ...
    that you calculate using `paddle.float64` (default is `paddle.float32`) which can be set using the `.set_dtype`
    method of the metric.

    .. hint::
        Using this metric with the default feature extractor requires that ``torch-fidelity``
        is installed. Either install as ``pip install paddlemetrics[image]`` or ``pip install torch-fidelity``

    As input to ``forward`` and ``update`` the metric accepts the following input

    - ``imgs`` (:class:`~paddle.Tensor`): tensor with images feed to the feature extractor with
    - ``real`` (:class:`~bool`): bool indicating if ``imgs`` belong to the real or the fake distribution

    As output of `forward` and `compute` the metric returns the following output

    - ``fid`` (:class:`~paddle.Tensor`): float scalar tensor with mean FID value over samples

    Args:
        feature:
            Either an integer or ``nn.Layer``:

            - an integer will indicate the inceptionv3 feature layer to choose. Can be one of the following:
              64, 192, 768, 2048
            - an ``nn.Layer`` for using a custom feature extractor. Expects that its forward method returns
        ...
        normalize:
            ...
            - If custom feature extractor module is used, controls type of the input img tensors:
              ...
              - True: if input imgs are expected to be in the data type of paddle.float32.
              - False: if input imgs are expected to be in the data type of paddle.int8.
    ...
    Raises:
        ...
        TypeError:
            If ``feature`` is not an ``str``, ``int`` or ``paddle.nn.Layer``
        ValueError:
    ...
    Example:
        ...
        >>> imgs_dist1 = paddle.randint(0, 200, (100, 3, 299, 299), dtype=paddle.uint8)
        >>> imgs_dist2 = paddle.randint(100, 255, (100, 3, 299, 299), dtype=paddle.uint8)
    ...
    inception: paddle.nn.Layer
    feature_network: str = "inception"

    def __init__(
        self,
        feature: Union[int, paddle.nn.Layer] = 2048,
        ...
        **kwargs: Any,
        ...
        if isinstance(feature, int):
            ...
            if not _TORCH_FIDELITY_AVAILABLE:
                raise ModuleNotFoundError(
                    "FrechetInceptionDistance metric requires that `Torch-fidelity` is installed. Either install as `pip install paddlemetrics[image]` or `pip install torch-fidelity`."
                )
        ...
        elif isinstance(feature, paddle.nn.Layer):
            ...
            if hasattr(self.inception, "num_features"):
                ...
                elif isinstance(self.inception.num_features, paddle.Tensor):
                    num_features = int(self.inception.num_features.item())
                else:
                    raise TypeError(
                        "Expected `self.inception.num_features` to be of type int or Tensor."
                    )
            else:
                if self.normalize:
                    dummy_image = paddle.rand(1, *input_img_size, dtype=paddle.float32)
                else:
                    dummy_image = paddle.randint(
                        low=0, high=255, shape=(1, *input_img_size), dtype=paddle.uint8
                    )
        ...
        self.add_state(
            "real_features_sum",
            paddle.zeros(num_features).double(),
            dist_reduce_fx="sum",
        )
        self.add_state(
            "real_features_cov_sum",
            paddle.zeros(mx_num_feats).double(),
            dist_reduce_fx="sum",
        )
        self.add_state(
            "real_features_num_samples", paddle.tensor(0).long(), dist_reduce_fx="sum"
        )
        self.add_state(
            "fake_features_sum",
            paddle.zeros(num_features).double(),
        ...
        self.add_state(
            "fake_features_num_samples", paddle.tensor(0).long(), dist_reduce_fx="sum"
        )

    def update(self, imgs: paddle.Tensor, real: bool) -> None:
    ...
    def set_dtype(self, dst_type: Union[str, paddle.dtype]) -> "Metric":
        ...
        Arguments:
            dst_type: the desired type as ``paddle.dtype`` or string
    ...
    def plot(
        ...
        .. plot::
            ...
            >>> import paddle
            >>> from paddlemetrics.image.fid import FrechetInceptionDistance
            >>> imgs_dist1 = lambda: paddle.randint(0, 200, (100, 3, 299, 299), dtype=paddle.uint8)
            >>> imgs_dist2 = lambda: paddle.randint(100, 255, (100, 3, 299, 299), dtype=paddle.uint8)
            >>> metric = FrechetInceptionDistance(feature=64)
            >>> values = [ ]
            >>> for _ in range(3):
            ...     metric.update(imgs_dist1(), real=True)
