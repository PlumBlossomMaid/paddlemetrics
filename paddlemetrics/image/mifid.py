...
from paddlemetrics.image.fid import NoTrainInceptionV3, _compute_fid
...
def _compute_cosine_distance(
    features1: paddle.Tensor, features2: paddle.Tensor, cosine_distance_eps: float = 0.1
) -> paddle.Tensor:
    """Compute the cosine distance between two sets of features."""
    features1_nozero = features1[paddle.sum(features1, axis=1) != 0]
    features2_nozero = features2[paddle.sum(features2, axis=1) != 0]
    norm_f1 = features1_nozero / paddle.norm(features1_nozero, axis=1, keepdim=True)
    norm_f2 = features2_nozero / paddle.norm(features2_nozero, axis=1, keepdim=True)
    d = 1.0 - paddle.abs(paddle.matmul(norm_f1, norm_f2.t()))
    mean_min_d = paddle.mean((d.min(axis=1), d.argmin(axis=1)).values)
    return (
        mean_min_d if mean_min_d < cosine_distance_eps else paddle.ones_like(mean_min_d)
    )


def _mifid_compute(
    mu1: paddle.Tensor,
    sigma1: paddle.Tensor,
    features1: paddle.Tensor,
    mu2: paddle.Tensor,
    sigma2: paddle.Tensor,
    features2: paddle.Tensor,
    ...
    return (
        fid_value / (distance + 1e-14)
        if fid_value > 1e-08
        else paddle.zeros_like(fid_value)
    )
...
class MemorizationInformedFrechetInceptionDistance(Metric):
    ...
    - ``mifid`` (:class:`~paddle.Tensor`): float scalar tensor with mean MIFID value over samples
    ...
    Raises:
        RuntimeError:
            If ``paddle`` is version less than 1.10
        ValueError:
    ...
    Example::
        ...
        >>> from paddlemetrics.image.mifid import MemorizationInformedFrechetInceptionDistance
        >>> mifid = MemorizationInformedFrechetInceptionDistance(feature=64)
        >>> # generate two slightly overlapping image intensity distributions
        >>> imgs_dist1 = randint(0, 200, (100, 3, 299, 299), dtype=paddle.uint8)
        >>> imgs_dist2 = randint(100, 255, (100, 3, 299, 299), dtype=paddle.uint8)
    ...
    real_features: List[paddle.Tensor]
    fake_features: List[paddle.Tensor]
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if isinstance(feature, int):
            if not _TORCH_FIDELITY_AVAILABLE:
                raise ModuleNotFoundError(
                    "MemorizationInformedFrechetInceptionDistance metric requires that `Torch-fidelity` is installed. Either install as `pip install paddlemetrics[image]` or `pip install torch-fidelity`."
                )
        ...
        if not (
            isinstance(cosine_distance_eps, float) and 1 >= cosine_distance_eps > 0
        ):
            raise ValueError(
                "Argument `cosine_distance_eps` expected to be a float greater than 0 and less than 1"
            )
    ...
    def compute(self) -> paddle.Tensor:
        ...
        mean_real, mean_fake = paddle.mean(real_features, axis=0), paddle.mean(
            fake_features, axis=0
        )
        cov_real, cov_fake = paddle.linalg.cov(x=real_features.t()), paddle.linalg.cov(
            x=fake_features.t()
        )
