...
from paddlemetrics.functional.image.sam import _sam_compute, _sam_update
...
class SpectralAngleMapper(Metric):
    ...
    - ``sam`` (:class:`~paddle.Tensor`): if ``reduction!='none'`` returns float scalar tensor with average SAM value
    ...
    Example:
        ...
        >>> from paddlemetrics.image import SpectralAngleMapper
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if reduction == "none" or reduction is None:
            rank_zero_warn(
                "Metric `SpectralAngleMapper` will save all targets and predictions in the buffer when using`reduction=None` or `reduction='none'. For large datasets, this may lead to a large memory footprint."
            )
            self.add_state("preds", default=[], dist_reduce_fx="cat")
            self.add_state("target", default=[], dist_reduce_fx="cat")
        else:
            self.add_state("sum_sam", paddle.tensor(0.0), dist_reduce_fx="sum")
