...
from paddlemetrics.functional.image.uqi import _uqi_compute, _uqi_update
...
class UniversalImageQualityIndex(Metric):
    ...
    - ``uiqi`` (:class:`~paddle.Tensor`): if ``reduction!='none'`` returns float scalar tensor with average UIQI value
    ...
    Example:
        >>> import paddle
        >>> from paddlemetrics.image import UniversalImageQualityIndex
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if reduction is None or reduction == "none":
            rank_zero_warn(
                "Metric `UniversalImageQualityIndex` will save all targets and predictions in the buffer when using`reduction=None` or `reduction='none'. For large datasets, this may lead to a large memory footprint."
            )
            self.add_state("preds", default=[], dist_reduce_fx="cat")
            self.add_state("target", default=[], dist_reduce_fx="cat")
        else:
            self.add_state("sum_uqi", paddle.tensor(0.0), dist_reduce_fx="sum")
            self.add_state("numel", paddle.tensor(0), dist_reduce_fx="sum")
