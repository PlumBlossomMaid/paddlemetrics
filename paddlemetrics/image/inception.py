...
class InceptionScore(Metric):
    ...
    - ``imgs`` (:class:`~paddle.Tensor`): tensor with images feed to the feature extractor

    As output of `forward` and `compute` the metric returns the following output

    - ``inception_mean`` (:class:`~paddle.Tensor`): float scalar tensor with mean inception score over subsets
    - ``inception_std`` (:class:`~paddle.Tensor`): float scalar tensor with standard deviation of inception score
      over subsets

    Args:
        feature:
            Either an str, integer or ``nn.Layer``:
    ...
    Example:
        ...
        >>> imgs = paddle.randint(0, 255, (100, 3, 299, 299), dtype=paddle.uint8)
    ...
    def __init__(
        self,
        feature: Union[str, int, paddle.nn.Layer] = "logits_unbiased",
        splits: int = 10,
        ...
        **kwargs: Any,
        ...
        rank_zero_warn(
            "Metric `InceptionScore` will save all extracted features in buffer. For large datasets this may lead to large memory footprint.",
            UserWarning,
        )
        if isinstance(feature, (str, int)):
            if not _TORCH_FIDELITY_AVAILABLE:
                raise ModuleNotFoundError(
                    "InceptionScore metric requires that `Torch-fidelity` is installed. Either install as `pip install paddlemetrics[image]` or `pip install torch-fidelity`."
                )
    ...
    def update(self, imgs: paddle.Tensor) -> None:
    ...
    def compute(self) -> tuple[paddle.Tensor, paddle.Tensor]:
        ...
        idx = paddle.randperm(features.shape[0])
        features = features[idx]
        prob = features.softmax(dim=1)
        log_prob = features.log_softmax(dim=1)
        prob = prob.chunk(self.splits, axis=0)
        log_prob = log_prob.chunk(self.splits, axis=0)
    ...
    def plot(
        ...
        .. plot::
            ...
            >>> metric.update(paddle.randint(0, 255, (50, 3, 299, 299), dtype=paddle.uint8))
        ...
        .. plot::
            ...
            ...     values.append(metric(paddle.randint(0, 255, (50, 3, 299, 299), dtype=paddle.uint8))[0])
