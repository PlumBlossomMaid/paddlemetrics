...
from paddlemetrics.functional.image.arniqa import (_ARNIQA,
...
class ARNIQA(Metric):
    ...
    - ``img`` (:class:`~paddle.Tensor`): tensor with images of shape ``(N, 3, H, W)``

    As output of `forward` and `compute` the metric returns the following output

    - ``arniqa`` (:class:`~paddle.Tensor`): tensor with ARNIQA score. If `reduction` is set to ``none``, the output will
    ...
    def __init__(
        ...
        **kwargs: Any) -> None:
        ...
        if not isinstance(normalize, bool):
            raise ValueError(
                f"Argument `normalize` should be a bool but got {normalize}"
            )
        self.normalize = normalize
        self.autocast = autocast
        self.add_state("sum_scores", paddle.tensor(0.0), dist_reduce_fx="sum")
        self.add_state("num_scores", paddle.tensor(0.0), dist_reduce_fx="sum")

    def update(self, img: paddle.Tensor) -> None:
        """Update internal states with arniqa score."""
        loss, num_scores = _arniqa_update(
            img, model=self.model, normalize=self.normalize, autocast=self.autocast
        )
    ...
    def plot(
        ...
        .. plot::
            ...
            >>> metric.update(paddle.rand(8, 3, 224, 224))
        ...
        .. plot::
            ...
            >>> values = [ ]
            >>> for _ in range(3):
            ...     values.append(metric(paddle.rand(8, 3, 224, 224)))
