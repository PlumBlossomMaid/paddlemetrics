...
from paddlemetrics.functional.image.d_s import (
    _spatial_distortion_index_compute, _spatial_distortion_index_update)
...
class QualityWithNoReference(Metric):
    ...
    - ``preds`` (:class:`~paddle.Tensor`): High resolution multispectral image of shape ``(N,C,H,W)``.
    - ``target`` (:class:`~Dict`): A dictionary containing the following keys:

      - ``ms`` (:class:`~paddle.Tensor`): Low resolution multispectral image of shape ``(N,C,H',W')``.
      - ``pan`` (:class:`~paddle.Tensor`): High resolution panchromatic image of shape ``(N,C,H,W)``.
      - ``pan_lr`` (:class:`~paddle.Tensor`): (optional) Low resolution panchromatic image of shape ``(N,C,H',W')``.
    ...
    - ``qnr`` (:class:`~paddle.Tensor`): if ``reduction!='none'`` returns float scalar tensor with average QNR value
    ...
    Example:
        ...
        >>> from paddlemetrics.image import QualityWithNoReference
    ...
    ms: List[paddle.Tensor]
    pan: List[paddle.Tensor]
    pan_lr: List[paddle.Tensor]
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        rank_zero_warn(
            "Metric `QualityWithNoReference` will save all targets and predictions in buffer. For large datasets this may lead to large memory footprint."
        )
    ...
    def update(self, preds: paddle.Tensor, target: dict[str, paddle.Tensor]) -> None:
        ...
        if "ms" not in target:
            raise ValueError(
                f"Expected `target` to have key `ms`. Got target: {target.keys()}."
            )
        if "pan" not in target:
            raise ValueError(
                f"Expected `target` to have key `pan`. Got target: {target.keys()}."
            )
        ms = target["ms"]
        pan = target["pan"]
    ...
    def compute(self) -> paddle.Tensor:
        ...
        d_lambda = _spectral_distortion_index_compute(
            preds, ms, self.norm_order, self.reduction
        )
