...
class SpatialDistortionIndex(Metric):
    ...
    :class:`~paddlemetrics.image.UniversalImageQualityIndex` for more info), :math:`\\\\hat{G_l}` is the l-th band of the
    ...
    - ``target`` (:class:`~Dict`): A dictionary containing the following keys:
        ...
        - ``pan_lr`` (:class:`~paddle.Tensor`): Low resolution panchromatic image of shape ``(N,C,H',W')``.
    ...
    Example:
        ...
        >>> from paddlemetrics.image import SpatialDistortionIndex
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        rank_zero_warn(
            "Metric `SpatialDistortionIndex` will save all targets and predictions in buffer. For large datasets this may lead to large memory footprint."
        )
