...
__doctest_requires__ = {
    ("KernelInceptionDistance", "KernelInceptionDistance.plot"): ["torch_fidelity"]
}


def maximum_mean_discrepancy(
    k_xx: paddle.Tensor, k_xy: paddle.Tensor, k_yy: paddle.Tensor
) -> paddle.Tensor:
    """Adapted from `KID Score`_."""
    m = k_xx.shape[0]
    diag_x = paddle.diag(k_xx)
    diag_y = paddle.diag(k_yy)
...
def poly_mmd(
    f_real: paddle.Tensor,
    f_fake: paddle.Tensor,
    degree: int = 3,
    gamma: Optional[float] = None,
    coef: float = 1.0,
) -> paddle.Tensor:
...
class KernelInceptionDistance(Metric):
    ...
    - ``imgs`` (:class:`~paddle.Tensor`): tensor with images feed to the feature extractor of shape ``(N,C,H,W)``
    - ``real`` (`bool`): bool indicating if ``imgs`` belong to the real or the fake distribution

    As output of `forward` and `compute` the metric returns the following output

    - ``kid_mean`` (:class:`~paddle.Tensor`): float scalar tensor with mean value over subsets
    - ``kid_std`` (:class:`~paddle.Tensor`): float scalar tensor with standard deviation value over subsets

    Args:
        feature: Either an str, integer or ``nn.Layer``:
    ...
    def __init__(
        self,
        feature: Union[str, int, paddle.nn.Layer] = 2048,
        subsets: int = 100,
        ...
        **kwargs: Any,
        ...
        rank_zero_warn(
            "Metric `Kernel Inception Distance` will save all extracted features in buffer. For large datasets this may lead to large memory footprint.",
            UserWarning,
        )
        ...
        if isinstance(feature, (str, int)):
            if not _TORCH_FIDELITY_AVAILABLE:
                raise ModuleNotFoundError(
                    "Kernel Inception Distance metric requires that `Torch-fidelity` is installed. Either install as `pip install paddlemetrics[image]` or `pip install torch-fidelity`."
                )
            ...
            self.inception: paddle.nn.Layer = NoTrainInceptionV3(
    ...
    def plot(
        ...
        .. plot::
            ...
            >>> import paddle
            >>> from paddlemetrics.image.kid import KernelInceptionDistance
            >>> imgs_dist1 = paddle.randint(0, 200, (30, 3, 299, 299), dtype=paddle.uint8)
            >>> imgs_dist2 = paddle.randint(100, 255, (30, 3, 299, 299), dtype=paddle.uint8)
        ...
        .. plot::
            ...
            >>> import paddle
            >>> from paddlemetrics.image.kid import KernelInceptionDistance
            >>> imgs_dist1 = lambda: paddle.randint(0, 200, (30, 3, 299, 299), dtype=paddle.uint8)
            >>> imgs_dist2 = lambda: paddle.randint(100, 255, (30, 3, 299, 299), dtype=paddle.uint8)
            >>> metric = KernelInceptionDistance(subsets=3, subset_size=20)
            >>> values = [ ]
            >>> for _ in range(3):
            ...     metric.update(imgs_dist1(), real=True)
