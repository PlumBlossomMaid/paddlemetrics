...
class HomogeneityScore(Metric):
    ...
    - ``rand_score`` (:class:`~paddle.Tensor`): A tensor with the Rand Score
    ...
    Example:
        >>> import paddle
        >>> from paddlemetrics.clustering import HomogeneityScore
...
class CompletenessScore(Metric):
    ...
    Example:
        >>> import paddle
        >>> from paddlemetrics.clustering import CompletenessScore
        >>> preds = paddle.to_tensor([2, 1, 0, 1, 0])
        >>> target = paddle.to_tensor([0, 2, 1, 1, 0])
...
class VMeasureScore(Metric):
    ...
    Example::
        >>> import paddle
        >>> from paddlemetrics.clustering import VMeasureScore
        >>> preds = paddle.to_tensor([2, 1, 0, 1, 0])
        >>> target = paddle.to_tensor([0, 2, 1, 1, 0])
