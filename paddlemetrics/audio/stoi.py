...
class ShortTimeObjectiveIntelligibility(Metric):
    ...
    - ``stoi`` (:class:`~paddle.Tensor`): float scalar tensor
    ...
    def __init__(self, fs: int, extended: bool = False, **kwargs: Any) -> None:
        ...
        if not _PYSTOI_AVAILABLE:
            raise ModuleNotFoundError(
                "STOI metric requires that `pystoi` is installed. Either install as `pip install paddlemetrics[audio]` or `pip install pystoi`."
            )
        self.fs = fs
        self.extended = extended
        self.add_state("sum_stoi", default=paddle.tensor(0.0), dist_reduce_fx="sum")
