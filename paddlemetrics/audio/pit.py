...
class PermutationInvariantTraining(Metric):
    ...
    - ``preds`` (:class:`~paddle.Tensor`): float tensor with shape ``(batch_size,num_speakers,...)``
    - ``target`` (:class:`~paddle.Tensor`): float tensor with shape ``(batch_size,num_speakers,...)``

    As output of `forward` and `compute` the metric returns the following output

    - ``pesq`` (:class:`~paddle.Tensor`): float scalar tensor with average PESQ value over samples
