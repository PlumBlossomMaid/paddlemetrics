...
if not all([_GAMMATONE_AVAILABLE, _TORCHAUDIO_AVAILABLE]):
    __doctest_skip__ = [
        ...
        "SpeechReverberationModulationEnergyRatio.plot",
    ]
...
class SpeechReverberationModulationEnergyRatio(Metric):
    ...
    - ``srmr`` (:class:`~paddle.Tensor`): float scaler tensor

    .. hint::
        Using this metrics requires you to have ``gammatone`` and ``torchaudio`` installed.
        Either install as ``pip install paddlemetrics[audio]`` or ``pip install torchaudio``
    ...
    Example:
        ...
        >>> from paddlemetrics.audio import SpeechReverberationModulationEnergyRatio
        >>> preds = randn(8000)
        ...
        **kwargs: Any
        ...
        if not _TORCHAUDIO_AVAILABLE or not _GAMMATONE_AVAILABLE:
            raise ModuleNotFoundError(
                "speech_reverberation_modulation_energy_ratio requires you to have `gammatone` and `torchaudio>=0.10` installed. Either install as ``pip install paddlemetrics[audio]`` or ``pip install torchaudio>=0.10`` and ``pip install git+https://github.com/detly/gammatone``"
            )
        _srmr_arg_validate(
            fs=fs,
    ...
    def update(self, preds: paddle.Tensor) -> None:
        ...
        self.total += metric_val_batch.size
