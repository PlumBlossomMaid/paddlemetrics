...
class MultilabelAccuracy(MultilabelStatScores):
    ...
    Example (preds is int tensor):
        >>> from paddle import tensor
        >>> from paddlemetrics.classification import MultilabelAccuracy
        >>> target = tensor([[0, 1, 0], [1, 0, 1]])
        >>> preds = tensor([[0, 0, 1], [1, 0, 1]])
...
class Accuracy(_ClassificationTaskWrapper):
    ...
    :class:`~paddlemetrics.classification.BinaryAccuracy`, :class:`~paddlemetrics.classification.MulticlassAccuracy` and
    :class:`~paddlemetrics.classification.MultilabelAccuracy` for the specific details of each argument influence and
    examples.
