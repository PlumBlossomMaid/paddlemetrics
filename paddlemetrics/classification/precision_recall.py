...
from paddlemetrics.functional.classification.precision_recall import _precision_recall_reduce
...
class BinaryPrecision(BinaryStatScores):
    ...
    - ``bp`` (:class:`~paddle.Tensor`): If ``multidim_average`` is set to ``global``, the metric returns a scalar
    ...
    Example (preds is int tensor):
        ...
        >>> from paddlemetrics.classification import BinaryPrecision
...
class MulticlassPrecision(MulticlassStatScores):
    ...
    - ``mcp`` (:class:`~paddle.Tensor`): The returned shape depends on the ``average`` and ``multidim_average``
      arguments:
    ...
    Example (preds is int tensor):
        >>> from paddle import tensor
        >>> from paddlemetrics.classification import MulticlassPrecision
...
class MultilabelPrecision(MultilabelStatScores):
    ...
    - ``preds`` (:class:`~paddle.Tensor`): An int tensor or float tensor of shape ``(N, C, ...)``.
    ...
    - ``mlp`` (:class:`~paddle.Tensor`): The returned shape depends on the ``average`` and ``multidim_average``
      arguments:
    ...
    Example (preds is int tensor):
        >>> from paddle import tensor
        >>> from paddlemetrics.classification import MultilabelPrecision
        >>> target = tensor([[0, 1, 0], [1, 0, 1]])
        >>> preds = tensor([[0, 0, 1], [1, 0, 1]])
...
class BinaryRecall(BinaryStatScores):
    ...
    - ``preds`` (:class:`~paddle.Tensor`): An int tensor or float tensor of shape ``(N, ...)``. If preds is a
    ...
    - ``br`` (:class:`~paddle.Tensor`): If ``multidim_average`` is set to ``global``, the metric returns a scalar
    ...
    Example (preds is int tensor):
        >>> from paddle import tensor
        >>> from paddlemetrics.classification import BinaryRecall
        >>> target = tensor([0, 1, 0, 1, 0, 1])
        >>> preds = tensor([0, 0, 1, 1, 0, 1])
        >>> metric = BinaryRecall()
        >>> metric(preds, target)
...
class MulticlassRecall(MulticlassStatScores):
    ...
    - ``mcr`` (:class:`~paddle.Tensor`): The returned shape depends on the ``average`` and ``multidim_average``
      arguments:
    ...
    Example (preds is int tensor):
        >>> from paddle import tensor
        >>> from paddlemetrics.classification import MulticlassRecall
        >>> target = tensor([2, 1, 0, 0])
        >>> preds = tensor([2, 1, 0, 1])
...
class MultilabelRecall(MultilabelStatScores):
    ...
    - ``mlr`` (:class:`~paddle.Tensor`): The returned shape depends on the ``average`` and ``multidim_average``
      arguments:
    ...
    Example (preds is int tensor):
        >>> from paddle import tensor
        >>> from paddlemetrics.classification import MultilabelRecall
        >>> target = tensor([[0, 1, 0], [1, 0, 1]])
        >>> preds = tensor([[0, 0, 1], [1, 0, 1]])
...
class Precision(_ClassificationTaskWrapper):
    ...
    :class:`~paddlemetrics.classification.BinaryPrecision`, :class:`~paddlemetrics.classification.MulticlassPrecision` and
    :class:`~paddlemetrics.classification.MultilabelPrecision` for the specific details of each argument influence and
    examples.
...
class Recall(_ClassificationTaskWrapper):
    ...
    This function is a simple wrapper to get the task specific versions of this metric, which is done by setting the
    ``task`` argument to either ``'binary'``, ``'multiclass'`` or ``'multilabel'``. See the documentation of
    :class:`~paddlemetrics.classification.BinaryRecall`,
    :class:`~paddlemetrics.classification.MulticlassRecall` and :class:`~paddlemetrics.classification.MultilabelRecall`
