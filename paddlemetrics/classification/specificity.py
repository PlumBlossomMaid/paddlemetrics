...
if not _MATPLOTLIB_AVAILABLE:
    __doctest_skip__ = [
        "BinarySpecificity.plot",
        "MulticlassSpecificity.plot",
        "MultilabelSpecificity.plot",
    ]
...
class BinarySpecificity(BinaryStatScores):
    ...
    - ``bs`` (:class:`~paddle.Tensor`): If ``multidim_average`` is set to ``global``, the metric returns a scalar value.
    ...
    Example (preds is int tensor):
        ...
        >>> from paddlemetrics.classification import BinarySpecificity
...
class MulticlassSpecificity(MulticlassStatScores):
    ...
    - ``mcs`` (:class:`~paddle.Tensor`): The returned shape depends on the ``average`` and ``multidim_average``
      arguments:
    ...
    Example (preds is int tensor):
        >>> from paddle import tensor
        >>> from paddlemetrics.classification import MulticlassSpecificity
...
class MultilabelSpecificity(MultilabelStatScores):
    ...
    - ``mls`` (:class:`~paddle.Tensor`): The returned shape depends on the ``average`` and ``multidim_average``
      arguments:
    ...
    Example (preds is int tensor):
        >>> from paddle import tensor
        >>> from paddlemetrics.classification import MultilabelSpecificity
        >>> target = tensor([[0, 1, 0], [1, 0, 1]])
        >>> preds = tensor([[0, 0, 1], [1, 0, 1]])
...
class Specificity(_ClassificationTaskWrapper):
    ...
    :class:`~paddlemetrics.classification.BinarySpecificity`, :class:`~paddlemetrics.classification.MulticlassSpecificity`
    and :class:`~paddlemetrics.classification.MultilabelSpecificity` for the specific details of each argument influence
    and examples.
