...
class PanopticQuality(Metric):
    """Compute the `Panoptic Quality`_ for panoptic segmentations.
    ...
    As input to ``forward`` and ``update`` the metric accepts the following input:
        ...
        - ``preds`` (:class:`~paddle.Tensor`): An int tensor of shape ``(B, *spatial_dims, 2)`` containing
          the pair ``(category_id, instance_id)`` for each point, where there needs to
          be at least one spatial dimension.
        - ``target`` (:class:`~paddle.Tensor`): An int tensor of shape ``(B, *spatial_dims, 2)`` containing
          the pair ``(category_id, instance_id)`` for each point, where there needs to
          be at least one spatial dimension.

    As output to ``forward`` and ``compute`` the metric returns the following output:

        - ``quality`` (:class:`~paddle.Tensor`): If ``return_sq_and_rq=False`` and ``return_per_class=False`` then a
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        self.add_state(
            "iou_sum",
            default=paddle.zeros(num_categories, dtype=paddle.float64),
            dist_reduce_fx="sum",
        )
        self.add_state(
            "true_positives",
            default=paddle.zeros(num_categories, dtype=paddle.int32),
            dist_reduce_fx="sum",
        )
        self.add_state(
            "false_positives",
