...
from paddlemetrics.functional.detection.diou import _diou_compute, _diou_update
...
if not _TORCHVISION_AVAILABLE:
    __doctest_skip__ = [
        ...
        "DistanceIntersectionOverUnion.plot",
    ]
...
class DistanceIntersectionOverUnion(IntersectionOverUnion):
    """Computes Distance Intersection Over Union (`DIoU`_).
    ...
    - ``diou_dict``: A dictionary containing the following key-values:
        ...
        - diou: (:class:`~paddle.Tensor`) with overall diou value over all classes and samples.
        - diou/cl_{cl}: (:class:`~paddle.Tensor`), if argument ``class_metrics=True``
    ...
    Example:
        >>> import paddle
        >>> from paddlemetrics.detection import DistanceIntersectionOverUnion
        >>> preds = [
        ...    {
