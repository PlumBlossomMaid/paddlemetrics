...
def _input_validator(
    ...
    for ivn in item_val_name:
        if not all(isinstance(target[ivn], paddle.Tensor) for target in targets):
            raise ValueError(f"Expected all {ivn} in `target` to be of type Tensor")
    if not all(isinstance(target["labels"], paddle.Tensor) for target in targets):
        raise ValueError("Expected all labels in `target` to be of type Tensor")
    for i, item in enumerate(targets):
        for ivn in item_val_name:
            if item[ivn].size(0) != item["labels"].size(0):
                raise ValueError(
                    f"Input '{ivn}' and labels of sample {i} in targets have a different length (expected {item[ivn].size(0)} labels, got {item['labels'].size(0)})"
                )
    if ignore_score:
        return
...
def _fix_empty_tensors(boxes: paddle.Tensor) -> paddle.Tensor:
...
def _load_coco_backend_tools(
    ...
    if backend == "pycocotools":
        if not _PYCOCOTOOLS_AVAILABLE:
            raise ModuleNotFoundError(
                "Backend `pycocotools` in metric `MeanAveragePrecision`  metric requires that `pycocotools` is installed. Please install with `pip install pycocotools` or `pip install paddlemetrics[detection]`"
            )
    ...
    if not _FASTER_COCO_EVAL_AVAILABLE:
        raise ModuleNotFoundError(
            "Backend `faster_coco_eval` in metric `MeanAveragePrecision`  metric requires that `faster-coco-eval` is installed. Please install with `pip install faster-coco-eval`."
        )
...
class CocoBackend:
    ...
    def _coco_stats_to_tensor_dict(
        ...
        return {
            f"{prefix}map": paddle.tensor([stats[0]], dtype=paddle.float32),
            f"{prefix}map_50": paddle.tensor([stats[1]], dtype=paddle.float32),
            f"{prefix}map_75": paddle.tensor([stats[2]], dtype=paddle.float32),
            f"{prefix}map_small": paddle.tensor([stats[3]], dtype=paddle.float32),
            f"{prefix}map_medium": paddle.tensor([stats[4]], dtype=paddle.float32),
            f"{prefix}map_large": paddle.tensor([stats[5]], dtype=paddle.float32),
            f"{prefix}mar_{mdt[0]}": paddle.tensor([stats[6]], dtype=paddle.float32),
            f"{prefix}mar_{mdt[1]}": paddle.tensor([stats[7]], dtype=paddle.float32),
            f"{prefix}mar_{mdt[2]}": paddle.tensor([stats[8]], dtype=paddle.float32),
            f"{prefix}mar_small": paddle.tensor([stats[9]], dtype=paddle.float32),
            f"{prefix}mar_medium": paddle.tensor([stats[10]], dtype=paddle.float32),
            f"{prefix}mar_large": paddle.tensor([stats[11]], dtype=paddle.float32),
        }
    ...
    def coco_to_tm(
        ...
        for t in gt_dataset:
            if t["image_id"] not in target:
                target[t["image_id"]] = {"labels": [], "iscrowd": [], "area": []}
                if "bbox" in iou_type:
        ...
        for key in target:
            bp = {
                "scores": paddle.tensor(preds[key]["scores"], dtype=paddle.float32),
            ...
            bt = {
                ...
                "iscrowd": paddle.tensor(target[key]["iscrowd"], dtype=paddle.int32),
                "area": paddle.tensor(target[key]["area"], dtype=paddle.float32),
            }
            if "bbox" in iou_type:
    ...
    def _get_coco_format(
        ...
        boxes: Optional[List[paddle.Tensor]] = None,
        masks: Optional[List[paddle.Tensor]] = None,
        ...
        for image_id, image_labels in enumerate(labels):
            ...
            for k, image_label in enumerate(image_labels):
                ...
                if not isinstance(image_label, int):
                    raise ValueError(
                        f"Invalid input class of sample {image_id}, element {k} (expected value of type integer, got type {type(image_label)})"
                    )
                area_stat_box = None
                area_stat_mask = None
                ...
                if scores is not None:
                    ...
                    if not isinstance(score, float):
                        raise ValueError(
                            f"Invalid input score of sample {image_id}, element {k} (expected value of type float, got type {type(score)})"
                        )
