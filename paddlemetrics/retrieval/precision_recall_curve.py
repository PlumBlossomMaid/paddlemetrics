...
from paddlemetrics.utils.plot import _AX_TYPE, _PLOT_OUT_TYPE, plot_curve

if not _MATPLOTLIB_AVAILABLE:
    __doctest_skip__ = [
        "RetrievalPrecisionRecallCurve.plot",
        "RetrievalRecallAtFixedPrecision.plot",
    ]
...
def _retrieval_recall_at_fixed_precision(
    ...
    if max_recall == 0.0:
        best_k = paddle.tensor(len(top_k), device=top_k.device, dtype=top_k.dtype)
...
class RetrievalPrecisionRecallCurve(Metric):
    ...
    - ``precisions`` (:class:`~paddle.Tensor`): A tensor with the fraction of relevant documents among all the
      retrieved documents.
    - ``recalls`` (:class:`~paddle.Tensor`): A tensor with the fraction of relevant documents retrieved among all the
      relevant documents
    - ``top_k`` (:class:`~paddle.Tensor`): A tensor with k from 1 to `max_k`
    ...
    Example:
        ...
        >>> target = tensor([True, False, False, False])
    ...
    def compute(self) -> tuple[paddle.Tensor, paddle.Tensor, paddle.Tensor]:
        ...
        top_k = paddle.arange(1, max_k + 1, device=preds.place)
        return precision, recall, top_k

    def plot(
        self,
        curve: Optional[tuple[paddle.Tensor, paddle.Tensor, paddle.Tensor]] = None,
...
class RetrievalRecallAtFixedPrecision(RetrievalPrecisionRecallCurve):
    ...
    - ``max_recall`` (:class:`~paddle.Tensor`): A tensor with the maximum recall value
      retrieved documents.
    - ``best_k`` (:class:`~paddle.Tensor`): A tensor with the best k corresponding to the maximum recall value
    ...
    Example:
        >>> from paddle import tensor
        >>> from paddlemetrics.retrieval import RetrievalRecallAtFixedPrecision
        >>> indexes = tensor([0, 0, 0, 0, 1, 1, 1])
        >>> preds = tensor([0.4, 0.01, 0.5, 0.6, 0.2, 0.3, 0.5])
        >>> target = tensor([True, False, False, False])
    ...
    def plot(
        ...
        .. plot::
            ...
            >>> values = []
            >>> for _ in range(10):
            ...     values.append(metric(paddle.rand(10,), paddle.randint(2, (10,)), indexes=paddle.randint(2,(10,)))[0])
