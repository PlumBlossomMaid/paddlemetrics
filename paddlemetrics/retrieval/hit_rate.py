...
class RetrievalHitRate(RetrievalMetric):
    ...
    - ``hr@k`` (:class:`~paddle.Tensor`): A single-value tensor with the hit rate (at ``top_k``) of the predictions
    ...
    Example:
        ...
        >>> from paddlemetrics.retrieval import RetrievalHitRate
        >>> indexes = tensor([0, 0, 0, 1, 1, 1, 1])
        >>> preds = tensor([0.2, 0.3, 0.5, 0.1, 0.3, 0.5, 0.2])
        >>> target = tensor([True, False, False, False, False])
