...
from unittests.text._inputs import (HYPOTHESIS_A, HYPOTHESIS_C,
...
def _reference_infolm_score(
    preds, target, model_name, information_measure, idf, alpha, beta
):
    ...
    precomputed_result = {
        "kl_divergence": paddle.tensor([-3.225, -0.1784, -0.1784, -2.2182]),
        "beta_divergence": paddle.tensor([0.5812, 0.0716, 0.0716, 0.3721]),
        "renyi_divergence": paddle.tensor([0.4357, 0.0333, 0.0333, 0.3615]),
        "l2_distance": paddle.tensor([0.2053, 0.1114, 0.1114, 0.2522]),
        "fisher_rao_distance": paddle.tensor([1.5637, 0.4957, 0.4957, 1.457]),
    }
    if len(preds) == 2:
        precomputed_result.update(
            {
                "alpha_divergence": paddle.tensor([-1.2851, -0.1262, -0.1262, -1.3096]),
                "ab_divergence": paddle.tensor([5.9517, 0.5222, 0.5222, 7.0017]),
                "l1_distance": paddle.tensor([0.9679, 0.1877, 0.1877, 0.9561]),
                "l_infinity_distance": paddle.tensor([0.0789, 0.0869, 0.0869, 0.2324]),
            }
        )
    elif len(preds) == 4:
        precomputed_result.update(
            {
                "alpha_divergence": paddle.tensor([-1.2893, -0.1262, -0.1262, -1.4035]),
                "ab_divergence": paddle.tensor([5.9565, 0.5222, 0.5222, 7.195]),
                "l1_distance": paddle.tensor([0.9591, 0.1877, 0.1877, 1.0823]),
                "l_infinity_distance": paddle.tensor([0.0777, 0.0869, 0.0869, 0.2614]),
            }
        )
    else:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("alpha_divergence", 0.4, 0.3),
        ("beta_divergence", False, None, 0.6),
        ("ab_divergence", 0.25, 0.25),
...
class TestInfoLM(TextTester):
    ...
    def test_infolm_class(
        self, ddp, preds, targets, information_measure, idf, alpha, beta
    ):
    ...
    def test_infolm_functional(
        self, preds, targets, information_measure, idf, alpha, beta
    ):
    ...
    def test_infolm_differentiability(
        self, preds, targets, information_measure, idf, alpha, beta
    ):
    ...
    def test_infolm_higher_is_better_property(
        ...
        assert (
            ...
            == metric._information_measure_higher_is_better[information_measure]
        )
