...
TUPLE_OF_REFERENCES = ((REFERENCE_1A, REFERENCE_2A), (REFERENCE_1B, REFERENCE_2B)), (
    (REFERENCE_1B, REFERENCE_2B),
    (REFERENCE_1C, REFERENCE_2C),
)
TUPLE_OF_HYPOTHESES = (HYPOTHESIS_A, HYPOTHESIS_B), (HYPOTHESIS_B, HYPOTHESIS_C)
_inputs_single_sentence_multiple_references = _Input(
    preds=[HYPOTHESIS_B], target=[[REFERENCE_1B, REFERENCE_2B]]
)
_inputs_multiple_references = _Input(
    preds=TUPLE_OF_HYPOTHESES, target=TUPLE_OF_REFERENCES
)
_inputs_single_sentence_single_reference = _Input(
    preds=HYPOTHESIS_B, target=REFERENCE_1B
)
...
_inputs_single_reference = _Input(
    preds=TUPLE_OF_HYPOTHESES, target=TUPLE_OF_SINGLE_REFERENCES
)
_logits_inputs_fp32 = _Input(
    preds=paddle.rand(
        NUM_BATCHES, BATCH_SIZE, EXTRA_DIM, NUM_CLASSES, dtype=paddle.float32
    ),
...
_logits_inputs_fp64 = _Input(
    preds=paddle.rand(
        NUM_BATCHES, BATCH_SIZE, EXTRA_DIM, NUM_CLASSES, dtype=paddle.float64
    ),
    target=paddle.randint(
        low=0, high=NUM_CLASSES, shape=(NUM_BATCHES, BATCH_SIZE, EXTRA_DIM)
    ),
...
_logits_inputs_fp32_with_mask = _Input(
    preds=_logits_inputs_fp32.preds, target=_target_with_mask
)
_logits_inputs_fp64_with_mask = _Input(
    preds=_logits_inputs_fp64.preds, target=_target_with_mask
)
