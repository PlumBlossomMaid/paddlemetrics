...
class TestSDR(MetricTester):
    ...
    def test_sdr_functional(self, preds, target):
        ...
        self.run_functional_metric_test(preds, target, signal_distortion_ratio, _reference_sdr_batch, metric_args={})
...
def test_on_real_audio():
    ...
    assert paddle.allclose(x=sdr.float(), y=paddle.tensor(0.2211), rtol=0.0001, atol=0.0001).item()
...
def test_too_low_precision():
    ...
    assert paddle.allclose(x=sdr_tm.mean(), y=paddle.tensor(sdr_bss).mean(), rtol=0.0001, atol=0.01).item()
