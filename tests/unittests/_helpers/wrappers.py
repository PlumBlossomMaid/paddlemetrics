...
_ERROR_CONNECTION_MESSAGE_PATTERNS = ("We couldn't connect to", "Connection error")


def skip_on_running_out_of_memory(
    reason: str = "Skipping test as it ran out of memory.",
):
...
def skip_on_connection_issues(
    reason: str = "Unable to load checkpoints from HuggingFace `transformers`.",
):
...
def skip_on_cuda_oom(
    reason: str = "Skipping test due to CUDA Out of Memory (OOM) error.",
):
