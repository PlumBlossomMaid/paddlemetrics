...
def _class_test(
    rank: int,
    world_size: int,
    preds: Union[paddle.Tensor, list, list[dict[str, paddle.Tensor]]],
    target: Union[paddle.Tensor, list, list[dict[str, paddle.Tensor]]],
    metric_class: Metric,
    ...
    **kwargs_update: Any,
    ...
    for i in range(rank, num_batches, world_size):
        ...
        if metric.dist_sync_on_step and check_dist_sync_on_step and rank == 0:
            if isinstance(preds, paddle.Tensor):
                ddp_preds = paddle.concat([preds[i + r] for r in range(world_size)]).cpu()
            else:
    ...
    if isinstance(preds, paddle.Tensor):
        total_preds = paddle.concat([preds[i] for i in range(num_batches)]).cpu()
    else:
        total_preds = [item for sublist in preds for item in sublist]
    if isinstance(target, paddle.Tensor):
        total_target = paddle.concat([target[i] for i in range(num_batches)]).cpu()
    elif isinstance(target, list) and len(target) > 0 and isinstance(target[0], dict):
        total_target = {k: paddle.concat([t[k] for t in target]) for k in target[0]}
    else:
...
def _functional_test(
    preds: Union[paddle.Tensor, list],
    target: Union[paddle.Tensor, list, list[dict[str, paddle.Tensor]]],
    ...
    **kwargs_update: Any,
    ...
    t_size = target.shape[0] if isinstance(target, paddle.Tensor) else len(target)
...
def _assert_dtype_support(
    ...
    preds: paddle.Tensor,
    target: Union[paddle.Tensor, list[dict[str, paddle.Tensor]]],
    device: str = "cpu",
...
class MetricTester:
    ...
    def run_class_metric_test(
        self,
        ddp: bool,
        preds: Union[paddle.Tensor, list[dict]],
        target: Union[paddle.Tensor, list[dict]],
        metric_class: Metric,
        reference_metric: Callable,
...
class DummyMetric(Metric):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.add_state("x", paddle.tensor(0.0), dist_reduce_fx="sum")
...
def remove_ignore_index(
    target: paddle.Tensor, preds: paddle.Tensor, ignore_index: Optional[int]
