...
class TestNDCG(RetrievalMetricTester):
    ...
    def test_class_metric(
        ...
        target = target if target.amin() >= 0 else target - target.amin()
    ...
    @pytest.mark.parametrize("k", [None, 1, 4, 10])
    def test_functional_metric(self, preds: paddle.Tensor, target: paddle.Tensor, k: int):
...
def test_corner_case_with_tied_scores():
    """See issue: https://github.com/Lightning-AI/paddlemetrics/issues/2022."""
