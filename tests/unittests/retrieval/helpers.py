...
def get_group_indexes(
    ...
    Args:
        indexes: a :class:`~paddle.Tensor` or `np.ndarray` of integers

    Return:
        A list of integer :class:`~paddle.Tensor`s or `np.ndarray`s

    Example:
        >>> indexes = paddle.to_tensor([0, 0, 0, 1, 1, 1, 1])
...
def _compute_sklearn_metric(
    preds: Union[paddle.Tensor, array],
    target: Union[paddle.Tensor, array],
...
_errors_test_functional_metric_parameters_default = {
    ...
    "argvalues": [
        (
            _irs_bad_sz_fn.preds,
            _irs_bad_sz_fn.target,
...
_errors_test_functional_metric_parameters_k = {
    ...
    "argvalues": [
        (
            _irs.preds,
            _irs.target,
            "`top_k` has to be a positive integer or None",
            {"top_k": -10},
        ),
        (
            _irs.preds,
...
_errors_test_functional_metric_parameters_adaptive_k = {
    ...
    "argvalues": [
        (
            _irs.preds,
            _irs.target,
            "`adaptive_k` has to be a boolean",
            {"adaptive_k": 10},
        ),
        (
            _irs.preds,
...
_default_metric_class_input_arguments_ignore_index = {
    "argnames": "indexes,preds,target",
    "argvalues": [(_irs_ii.indexes, _irs_ii.preds, _irs_ii.target)],
}
