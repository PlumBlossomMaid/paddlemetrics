...
import pytest
from unittests._helpers import (_TORCH_LESS_THAN_2_1,
                                _TRANSFORMERS_RANGE_GE_4_50_LT_4_54, seed_all,
                                skip_on_connection_issues, skip_on_cuda_oom)
from unittests._helpers.testers import MetricTester

from paddlemetrics.functional.multimodal.clip_score import (
    _detect_modality, _get_clip_model_and_processor, _process_image_data,
    _process_text_data, clip_score)
...
_random_input = _InputImagesCaptions(
    images=paddle.randint(low=0, high=255, shape=(2, 2, 3, 64, 64)),
...
def _reference_clip_score(preds, target, model_name_or_path):
    ...
    inputs = processor(
        text=target, images=[p.cpu() for p in preds], return_tensors="pt", padding=True
    )
...
class TestCLIPScore(MetricTester):
    ...
    def test_clip_score(self, inputs, model_name_or_path, ddp):
        ...
        self.run_class_metric_test(
            ...
            reference_metric=partial(
                _reference_clip_score, model_name_or_path=model_name_or_path
            ),
    ...
    def test_error_on_not_same_amount_of_input(self, inputs, model_name_or_path):
        ...
        with pytest.raises(
            ValueError,
            match="Expected the number of source and target examples to be the same.*",
        ):
            metric(
                paddle.randint(low=0, high=255, shape=(2, 3, 64, 64)),
                "28-year-old chef found dead in San Francisco mall",
            )
    ...
    def test_error_on_wrong_image_format(self, inputs, model_name_or_path):
        ...
        with pytest.raises(
            ...
            metric(
                paddle.randint(low=0, high=255, shape=(64, 64)),
                "28-year-old chef found dead in San Francisco mall",
            )
    ...
    def test_clip_score_image_to_image(self, inputs, model_name_or_path):
        ...
        assert score.detach().round() == paddle.tensor(
            expected[
                model_name_or_path
                if not callable(model_name_or_path)
                else "_custom_clip_processor_model"
            ]
        )
    ...
    def test_clip_score_functional_image_to_image(self, inputs, model_name_or_path):
        ...
        preds, _ = inputs
        score = clip_score(
            preds[0][0], preds[0][1], model_name_or_path=model_name_or_path
        )
        assert score.detach().round() == paddle.tensor(
            expected[
                model_name_or_path
    ...
    def test_clip_score_functional_text_to_text(self, inputs, model_name_or_path):
        ...
        _, target = inputs
        score = clip_score(
            target[0][0], target[0][1], model_name_or_path=model_name_or_path
        )
        assert score.detach().round() == paddle.tensor(
            expected[
                model_name_or_path
...
@pytest.mark.parametrize(
    ...
    [
        (paddle.randn(3, 64, 64), 1, False),
        (paddle.randn(2, 3, 64, 64), 2, False),
        ([paddle.randn(3, 64, 64)], 1, False),
        ([paddle.randn(3, 64, 64), paddle.randn(3, 64, 64)], 2, False),
        (paddle.randn(64, 64), 0),
        ([paddle.randn(64, 64)], 0),
    ],
)
def test_process_image_data(images, expected_len, should_raise):
    ...
    if should_raise:
        ...
        assert all(
            isinstance(img, paddle.Tensor) and img.ndim == 3 for img in processed
        )


@pytest.mark.parametrize(
    ("texts", "expected_len"),
    [("single text", 1), (["text1", "text2"], 2), ([""], 1), ([], 0)],
)
