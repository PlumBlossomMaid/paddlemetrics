...
class TestDiceScore(MetricTester):
    ...
    def test_dice_score_functional(
        self,
        preds,
        target,
        input_format,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("none", "samplewise", [paddle.nan, 0.4, paddle.nan, 0.1]),
        ("micro", "global", 0.25),
        ("macro", "global", 0.2909),
        ("weighted", "global", 0.249),
        ("none", "global", [paddle.nan, 0.4, paddle.nan, 0.1818]),
    ],
)
def test_samples_with_missing_classes(average, aggregation_level, expected_score):
    ...
    target = paddle.full((num_samples, num_classes, 10, 10), 0, dtype=paddle.int8)
    preds = paddle.full((num_samples, num_classes, 10, 10), 0, dtype=paddle.int8)
...
def test_corner_case_zero_denominator(aggregation_level, average):
    ...
    num_classes = 3
    target = paddle.full((4, num_classes, 128, 128), 0, dtype=paddle.int8)
    preds = paddle.full((4, num_classes, 128, 128), 0, dtype=paddle.int8)
    dice = DiceScore(
        num_classes=num_classes,
        average=average,
        include_background=True,
