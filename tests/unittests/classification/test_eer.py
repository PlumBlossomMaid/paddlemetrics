...
from paddlemetrics.classification.eer import (EER, BinaryEER, MulticlassEER,
                                             MultilabelEER)
from paddlemetrics.functional.classification.eer import (binary_eer,
                                                        multiclass_eer,
                                                        multilabel_eer)
...
class TestBinaryEER(MetricTester):
    ...
    def test_binary_eer_functional(self, inputs, ignore_index):
        ...
        self.run_functional_metric_test(
            ...
            reference_metric=partial(
                _reference_sklearn_eer_binary, ignore_index=ignore_index
            ),
    ...
    def test_binary_eer_threshold_arg(self, inputs, threshold_fn):
        ...
        for pred, true in zip(preds, target):
            ...
            ap2 = binary_eer(pred, true, thresholds=threshold_fn(t.flip(axis=0)))
...
def _reference_sklearn_eer_multilabel(preds, target, ignore_index=None):
    eer = [
        _reference_sklearn_eer_binary(preds[:, i], target[:, i], ignore_index)
        for i in range(NUM_CLASSES)
    ]
    return np.array(eer)
...
class TestMultilabelEER(MetricTester):
    ...
    def test_multilabel_eer_functional(self, inputs, ignore_index):
        ...
        self.run_functional_metric_test(
            ...
            reference_metric=partial(
                _reference_sklearn_eer_multilabel, ignore_index=ignore_index
            ),
            metric_args={
                "thresholds": None,
