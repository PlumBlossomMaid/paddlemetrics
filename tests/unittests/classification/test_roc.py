...
class TestMulticlassROC(MetricTester):
        ...
        for pred, true in zip(preds, target):
            ...
            for i, t in enumerate(t1):
                p2, r2, t2 = multiclass_roc(
                    ...
                    true,
                    num_classes=NUM_CLASSES,
                    thresholds=threshold_fn(t.flip(axis=0)),
                )
                assert paddle.allclose(x=p1[i], y=p2[i]).item()
                assert paddle.allclose(x=r1[i], y=r2[i]).item()
                assert paddle.allclose(x=t1[i], y=t2).item()
    ...
    def test_multiclass_average(self, inputs, average, thresholds):
        ...
        assert all(isinstance(o, paddle.Tensor) for o in output)
        none_output = multiclass_roc(
            preds[0],
            target[0],
            num_classes=NUM_CLASSES,
            thresholds=thresholds,
