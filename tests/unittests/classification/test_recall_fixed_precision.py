...
def _reference_sklearn_recall_at_fixed_precision_binary(
    preds, target, min_precision, ignore_index=None
):
...
class TestBinaryRecallAtFixedPrecision(MetricTester):
    ...
    def test_binary_recall_at_fixed_precision(
        self, inputs, ddp, min_precision, ignore_index
    ):
    ...
    @pytest.mark.parametrize("ignore_index", [None, -1, 0])
    def test_binary_recall_at_fixed_precision_functional(
        self, inputs, min_precision, ignore_index
    ):
    ...
    @pytest.mark.parametrize("min_precision", [0.05, 0.5, 0.8])
    def test_binary_recall_at_fixed_precision_threshold_arg(
        self, inputs, min_precision
    ):
        """Test that different types of `thresholds` argument lead to same result."""
        preds, target = inputs
        for pred, true in zip(preds, target):
            pred = paddle.tensor(np.round(pred.numpy(), 1)) + 1e-06
            r1, _ = binary_recall_at_fixed_precision(
                pred, true, min_precision=min_precision, thresholds=None
            )
...
def _reference_sklearn_recall_at_fixed_precision_multiclass(
    preds, target, min_precision, ignore_index=None
):
...
class TestMulticlassRecallAtFixedPrecision(MetricTester):
    ...
    @pytest.mark.parametrize("min_precision", [0.05, 0.5, 0.8])
    @pytest.mark.parametrize("ignore_index", [None, -1, 0])
    def test_multiclass_recall_at_fixed_precision_functional(
        self, inputs, min_precision, ignore_index
    ):
        """Test functional implementation of metric."""
        preds, target = inputs
    ...
    @pytest.mark.parametrize("min_precision", [0.05, 0.5, 0.8])
    def test_multiclass_recall_at_fixed_precision_threshold_arg(
        self, inputs, min_precision
    ):
        """Test that different types of `thresholds` argument lead to same result."""
        preds, target = inputs
...
def _reference_sklearn_recall_at_fixed_precision_multilabel(
    preds, target, min_precision, ignore_index=None
):
    recall, thresholds = [], []
    for i in range(NUM_CLASSES):
...
class TestMultilabelRecallAtFixedPrecision(MetricTester):
    ...
    @pytest.mark.parametrize("ignore_index", [None, -1, 0])
    @pytest.mark.parametrize("ddp", [pytest.param(True, marks=pytest.mark.DDP), False])
    def test_multilabel_recall_at_fixed_precision(
        self, inputs, ddp, min_precision, ignore_index
    ):
        """Test class implementation of metric."""
        preds, target = inputs
    ...
    @pytest.mark.parametrize("min_precision", [0.05, 0.5, 0.8])
    @pytest.mark.parametrize("ignore_index", [None, -1, 0])
    def test_multilabel_recall_at_fixed_precision_functional(
        self, inputs, min_precision, ignore_index
    ):
        """Test functional implementation of metric."""
        preds, target = inputs
    ...
    @pytest.mark.parametrize("min_precision", [0.05, 0.5, 0.8])
    def test_multilabel_recall_at_fixed_precision_threshold_arg(
        self, inputs, min_precision
    ):
        """Test that different types of `thresholds` argument lead to same result."""
        preds, target = inputs
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {"task": "multiclass", "num_classes": 3, "min_precision": 0.5},
        ),
        (
            MultilabelRecallAtFixedPrecision,
            {"task": "multilabel", "num_labels": 3, "min_precision": 0.5},
        ),
...
def test_binary_recall_at_fixed_precision_nan_threshold():
    ...
    assert recall == 0.0
    assert paddle.isnan(threshold), "Expected NaN when no precision condition is met"
