...
from paddlemetrics.functional.classification.hamming import (
    binary_hamming_distance, multiclass_hamming_distance,
...
def _reference_sklearn_hamming_distance_binary(
...
class TestBinaryHammingDistance(MetricTester):
    ...
    def test_binary_hamming_distance(self, ddp, inputs, ignore_index, multidim_average):
        ...
        self.run_class_metric_test(
            ...
            reference_metric=partial(
                _reference_sklearn_hamming_distance_binary,
    ...
    @pytest.mark.parametrize("ignore_index", [None, -1])
    @pytest.mark.parametrize("multidim_average", ["global", "samplewise"])
    def test_binary_hamming_distance_functional(
...
def _reference_sklearn_hamming_distance_multiclass_global(
...
def _reference_sklearn_hamming_distance_multiclass_local(
    preds, target, ignore_index, average
):
    preds = preds.numpy()
    target = target.numpy()
    ...
    for pred, true in zip(preds, target):
        ...
        if average == "micro":
            ...
            if average == "macro":
                ...
                res.append(
                    hamming_per_class.mean() if len(hamming_per_class) > 0 else 0.0
                )
...
def _reference_sklearn_hamming_distance_multiclass(
    preds, target, ignore_index, multidim_average, average
):
    if preds.ndim == target.ndim + 1:
        preds = paddle.argmax(preds, 1)
    if multidim_average == "global":
        return _reference_sklearn_hamming_distance_multiclass_global(
            preds, target, ignore_index, average
        )
    return _reference_sklearn_hamming_distance_multiclass_local(
        preds, target, ignore_index, average
    )
...
def _reference_sklearn_hamming_distance_multilabel_global(
    preds, target, ignore_index, average
):
    if average == "micro":
        preds = preds.flatten()
...
def _reference_sklearn_hamming_distance_multilabel_local(
    preds, target, ignore_index, average
):
    hamming, weights = [], []
...
def _reference_sklearn_hamming_distance_multilabel(
    preds, target, ignore_index, multidim_average, average
):
    preds = preds.numpy()
    target = target.numpy()
    ...
    if multidim_average == "global":
        return _reference_sklearn_hamming_distance_multilabel_global(
            preds, target, ignore_index, average
        )
    return _reference_sklearn_hamming_distance_multilabel_local(
        preds, target, ignore_index, average
    )
