...
_input_multilabel_multidim_prob = _Input(
    ...
    target=paddle.randint(low=0, high=2, shape=(NUM_BATCHES, BATCH_SIZE, NUM_CLASSES, EXTRA_DIM)),
)
...
_input_multilabel_multidim = _Input(
    preds=paddle.randint(low=0, high=2, shape=(NUM_BATCHES, BATCH_SIZE, NUM_CLASSES, EXTRA_DIM)),
    target=paddle.randint(low=0, high=2, shape=(NUM_BATCHES, BATCH_SIZE, NUM_CLASSES, EXTRA_DIM)),
)
_binary_cases = (
    pytest.param(
    ...
    pytest.param(
        _Input(
            preds=paddle.randint(low=0, high=2, shape=(NUM_BATCHES, BATCH_SIZE, EXTRA_DIM)),
...
_multiclass_cases = (
    ...
    pytest.param(
        _Input(
            preds=paddle.randint(low=0, high=NUM_CLASSES, shape=(NUM_BATCHES, BATCH_SIZE, EXTRA_DIM)),
            target=paddle.randint(low=0, high=NUM_CLASSES, shape=(NUM_BATCHES, BATCH_SIZE, EXTRA_DIM)),
        ),
        id="input[multi_dim-labels]",
    ),
    pytest.param(
        _Input(
            preds=paddle.rand(NUM_BATCHES, BATCH_SIZE, NUM_CLASSES, EXTRA_DIM).softmax(-2),
            target=paddle.randint(low=0, high=NUM_CLASSES, shape=(NUM_BATCHES, BATCH_SIZE, EXTRA_DIM)),
        ),
        id="input[multi_dim-probs]",
    ),
    pytest.param(
        _Input(
            preds=_logsoftmax(paddle.rand(NUM_BATCHES, BATCH_SIZE, NUM_CLASSES, EXTRA_DIM), -2),
            target=paddle.randint(low=0, high=NUM_CLASSES, shape=(NUM_BATCHES, BATCH_SIZE, EXTRA_DIM)),
        ),
        id="input[multi_dim-logits]",
    ),
...
_multilabel_cases = (
    ...
    pytest.param(
        _Input(
            preds=_inv_sigmoid(paddle.rand(NUM_BATCHES, BATCH_SIZE, NUM_CLASSES, EXTRA_DIM)),
            target=paddle.randint(low=0, high=2, shape=(NUM_BATCHES, BATCH_SIZE, NUM_CLASSES, EXTRA_DIM)),
        ),
        id="input[multi_dim-logits]",
    ),
...
__temp_preds = paddle.randint(low=0, high=2, shape=(NUM_BATCHES, BATCH_SIZE, NUM_CLASSES))
...
def _generate_plausible_inputs_multilabel(num_classes=NUM_CLASSES, num_batches=NUM_BATCHES, batch_size=BATCH_SIZE):
    correct_targets = paddle.randint(low=0, high=num_classes, shape=(num_batches, batch_size))
    preds = paddle.rand(num_batches, batch_size, num_classes)
    targets = paddle.zeros_like(preds, dtype=paddle.long)
    for i in range(preds.shape[0]):
        for j in range(preds.shape[1]):
            targets[i, j, correct_targets[i, j]] = 1
    preds += paddle.rand(num_batches, batch_size, num_classes) * targets.cast(paddle.float32) / 3
...
def _generate_plausible_inputs_binary(num_batches=NUM_BATCHES, batch_size=BATCH_SIZE):
    ...
    preds = (
        paddle.rand(num_batches, batch_size) + paddle.rand(num_batches, batch_size) * targets.cast(paddle.float32) / 3
    )
    return _Input(preds=preds / (preds.amax() + 0.01), target=targets)
...
_class_remove, _class_replace = paddle.multinomial(paddle.ones(NUM_CLASSES), num_samples=2, replacement=False)
