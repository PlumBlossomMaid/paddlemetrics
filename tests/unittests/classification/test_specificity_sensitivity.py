...
from paddlemetrics.functional.classification.specificity_sensitivity import (
    _convert_fpr_to_specificity, binary_specificity_at_sensitivity,
...
def _specificity_at_sensitivity_x_multilabel(predictions, targets, min_sensitivity):
    ...
    if not indices.any():
        max_spec, best_threshold = 0.0, 1000000.0
    else:
...
def _reference_sklearn_specificity_at_sensitivity_binary(
    preds, target, min_sensitivity, ignore_index=None
):
...
class TestBinarySpecificityAtSensitivity(MetricTester):
    ...
    def test_binary_specificity_at_sensitivity(
        self, inputs, ddp, min_sensitivity, ignore_index
    ):
    ...
    @pytest.mark.parametrize("ignore_index", [None, -1, 0])
    def test_binary_specificity_at_sensitivity_functional(
        self, inputs, min_sensitivity, ignore_index
    ):
    ...
    @pytest.mark.parametrize("min_sensitivity", [0.05, 0.1, 0.3, 0.5, 0.8])
    def test_binary_specificity_at_sensitivity_threshold_arg(
        self, inputs, min_sensitivity
    ):
        """Test that different types of `thresholds` argument lead to same result."""
        preds, target = inputs
        for pred, true in zip(preds, target):
            pred = paddle.tensor(np.round(pred.numpy(), 1)) + 1e-06
            r1, _ = binary_specificity_at_sensitivity(
                pred, true, min_sensitivity=min_sensitivity, thresholds=None
            )
...
def _reference_sklearn_specificity_at_sensitivity_multiclass(
    preds, target, min_sensitivity, ignore_index=None
):
    ...
    for i in range(NUM_CLASSES):
        ...
        res = _specificity_at_sensitivity_x_multilabel(
            preds[:, i], target_temp, min_sensitivity
        )
...
class TestMulticlassSpecificityAtSensitivity(MetricTester):
    ...
    @pytest.mark.parametrize("ignore_index", [None, -1, 0])
    @pytest.mark.parametrize("ddp", [pytest.param(True, marks=pytest.mark.DDP), False])
    def test_multiclass_specificity_at_sensitivity(
        self, inputs, ddp, min_sensitivity, ignore_index
    ):
        """Test class implementation of metric."""
        preds, target = inputs
    ...
    @pytest.mark.parametrize("min_sensitivity", [0.05, 0.1, 0.3, 0.5, 0.8])
    @pytest.mark.parametrize("ignore_index", [None, -1, 0])
    def test_multiclass_specificity_at_sensitivity_functional(
        self, inputs, min_sensitivity, ignore_index
    ):
        """Test functional implementation of metric."""
        preds, target = inputs
    ...
    @pytest.mark.parametrize("min_sensitivity", [0.05, 0.1, 0.3, 0.5, 0.8])
    def test_multiclass_specificity_at_sensitivity_threshold_arg(
        self, inputs, min_sensitivity
    ):
        """Test that different types of `thresholds` argument lead to same result."""
        preds, target = inputs
        ...
        for pred, true in zip(preds, target):
            pred = paddle.tensor(np.round(pred.detach().numpy(), 1)) + 1e-06
...
def _reference_sklearn_specificity_at_sensitivity_multilabel(
    preds, target, min_sensitivity, ignore_index=None
):
    specificity, thresholds = [], []
    for i in range(NUM_CLASSES):
...
class TestMultilabelSpecificityAtSensitivity(MetricTester):
    ...
    @pytest.mark.parametrize("ignore_index", [None, -1, 0])
    @pytest.mark.parametrize("ddp", [pytest.param(True, marks=pytest.mark.DDP), False])
    def test_multilabel_specificity_at_sensitivity(
        self, inputs, ddp, min_sensitivity, ignore_index
    ):
        """Test class implementation of metric."""
        preds, target = inputs
    ...
    @pytest.mark.parametrize("min_sensitivity", [0.05, 0.1, 0.3, 0.5, 0.8])
    @pytest.mark.parametrize("ignore_index", [None, -1, 0])
    def test_multilabel_specificity_at_sensitivity_functional(
        self, inputs, min_sensitivity, ignore_index
    ):
        """Test functional implementation of metric."""
        preds, target = inputs
    ...
    @pytest.mark.parametrize("min_sensitivity", [0.05, 0.1, 0.3, 0.5, 0.8])
    def test_multilabel_specificity_at_sensitivity_threshold_arg(
        self, inputs, min_sensitivity
    ):
        """Test that different types of `thresholds` argument lead to same result."""
        preds, target = inputs
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {"task": "multiclass", "num_classes": 3, "min_sensitivity": 0.5},
        ),
        (
            MultilabelSpecificityAtSensitivity,
            {"task": "multilabel", "num_labels": 3, "min_sensitivity": 0.5},
        ),
