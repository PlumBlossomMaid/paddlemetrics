...
from paddlemetrics.image.fid import FrechetInceptionDistance, NoTrainInceptionV3
...
def test_no_train_network_missing_torch_fidelity(monkeypatch):
    ...
    monkeypatch.setattr("paddlemetrics.image.fid._TORCH_FIDELITY_AVAILABLE", False)
    with pytest.raises(
        ModuleNotFoundError,
        match="NoTrainInceptionV3 module requires that `Torch-fidelity` is installed.*",
    ):
...
def test_fid_same_input(feature):
    ...
    assert paddle.allclose(
        x=metric.real_features_sum, y=metric.fake_features_sum
    ).item()
    assert paddle.allclose(
        x=metric.real_features_cov_sum, y=metric.fake_features_cov_sum
    ).item()
    assert paddle.allclose(
        x=metric.real_features_num_samples, y=metric.fake_features_num_samples
    ).item()
    val = metric.compute()
...
def test_compare_fid(tmpdir, equal_size, feature=768):
    ...
    assert paddle.allclose(
        x=tm_res.cpu(),
        y=paddle.tensor([torch_fid["frechet_inception_distance"]]),
        atol=0.001,
    ).item()
...
def test_reset_real_features_arg(reset_real_features):
    ...
    assert metric.real_features_sum.shape == paddle.Size([64])
    assert metric.real_features_cov_sum.shape == paddle.Size([64, 64])
    assert metric.fake_features_num_samples == 2
    assert metric.fake_features_sum.shape == paddle.Size([64])
    assert metric.fake_features_cov_sum.shape == paddle.Size([64, 64])
    metric.reset()
...
def test_not_enough_samples():
    ...
    img = paddle.randint(low=0, high=255, shape=(1, 3, 299, 299), dtype=paddle.uint8)
    metric = FrechetInceptionDistance()
    metric.update(img, real=True)
    metric.update(img, real=False)
    with pytest.raises(
...
def test_dtype_transfer_to_submodule():
    ...
    imgs = paddle.randn(1, 3, 256, 256)
    imgs = ((imgs.clamp(-1, 1) / 2 + 0.5) * 255).to(paddle.uint8)
    metric = FrechetInceptionDistance(feature=64)
    metric.set_dtype(paddle.float64)
...
def test_antialias():
    ...
    imgs = paddle.randint(low=0, high=255, shape=(10, 3, 299, 299), dtype=paddle.uint8)
    ...
    assert paddle.allclose(x=val_no_aa, y=val_aa, atol=0.001).item()
