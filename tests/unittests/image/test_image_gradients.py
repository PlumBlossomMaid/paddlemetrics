...
def test_invalid_input_ndims(batch_size=1, height=5, width=5, channels=1):
    ...
    image = paddle.arange(0, batch_size * height * width * channels, dtype=paddle.float32)
...
def test_multi_batch_image_gradients(batch_size=5, height=5, width=5, channels=1):
    ...
    single_channel_img = paddle.arange(0, 1 * height * width * channels, dtype=paddle.float32)
