...
_multiclass_randint_input = lambda: paddle.randint(low=0, high=3, shape=(10,))
_multiclass_randn_input = lambda: paddle.randn(10, 3).softmax(dim=-1)
_multilabel_rand_input = lambda: paddle.rand(10, 3)
_multilabel_randint_input = lambda: paddle.randint(low=0, high=2, shape=(10, 3))
_audio_input = lambda: paddle.randn(8000)
_image_input = lambda: paddle.rand([8, 3, 16, 16])
_panoptic_input = lambda: paddle.multinomial(
    paddle.tensor([1, 1, 0, 0, 0, 0, 1, 1]).float(), 40, replacement=True
).reshape(1, 5, 4, 2)
_nominal_input = lambda: paddle.randint(low=0, high=4, shape=(100,))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(BinaryEER, _rand_input, _binary_randint_input, id="binary eer"),
        pytest.param(
        ...
        pytest.param(
            partial(TheilsU, num_classes=5),
            _nominal_input,
            _nominal_input,
            id="theils U",
        ),
        pytest.param(
            partial(TschuprowsT, num_classes=5),
            _nominal_input,
            _nominal_input,
            id="tschuprows T",
        ),
        pytest.param(
            partial(CramersV, num_classes=5),
            _nominal_input,
            _nominal_input,
            id="cramers V",
        ),
        pytest.param(
            partial(FleissKappa, mode="probs"),
            lambda: paddle.randn(10, 3, 5),
            None,
            id="fleiss kappa",
        ),
        pytest.param(
        ...
        pytest.param(
            ...
            lambda: paddle.tensor([[0.0, 1.0], [2.0, 3.0]]),
        ...
        pytest.param(
            MultiScaleStructuralSimilarityIndexMeasure,
            lambda: paddle.rand(3, 3, 180, 180),
            lambda: paddle.rand(3, 3, 180, 180),
        ...
        pytest.param(
            ...
            _rand_input,
            _rand_input,
            id="normalized root mean squared error",
        ),
        ...
        pytest.param(
            ...
            lambda: paddle.randint(low=0, high=3, shape=(20, 5)),
            lambda: paddle.randint(low=0, high=3, shape=(20, 5)),
            id="multiclass exact match",
        ),
        pytest.param(
            partial(MultilabelExactMatch, num_labels=3),
            lambda: paddle.randint(low=0, high=2, shape=(20, 3, 5)),
            lambda: paddle.randint(low=0, high=2, shape=(20, 3, 5)),
        ...
        pytest.param(
            ...
            lambda: paddle.randint(low=0, high=5, shape=(2, 8)),
            id="perplexity",
        ),
        pytest.param(
            ROUGEScore,
            lambda: "My name is John",
        ...
        pytest.param(
            ...
            lambda: [
                {
                    "answers": {"answer_start": [97], "text": ["1976"]},
...
@pytest.mark.parametrize(
    ("prefix", "postfix"),
    [(None, None), ("prefix", None), (None, "postfix"), ("prefix", "postfix")],
)
def test_plot_method_collection(together, num_vals, prefix, postfix):
    ...
    if num_vals == 1:
        m_collection.update(
            paddle.randint(low=0, high=2, shape=(10,)),
            paddle.randint(low=0, high=2, shape=(10,)),
        )
    ...
    with pytest.raises(
        ValueError,
        match="Expected argument `ax` to be a sequence of matplotlib axis objects with.*",
    ):
        m_collection.plot(ax=ax.tolist())
    plt.close(fig)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            BinaryROC,
            lambda: paddle.rand(100),
            lambda: paddle.randint(low=0, high=2, shape=(100,)),
            id="binary roc",
        ),
        pytest.param(
            partial(MulticlassROC, num_classes=3),
            lambda: paddle.randn(100, 3).softmax(dim=-1),
            lambda: paddle.randint(low=0, high=3, shape=(100,)),
            id="multiclass roc",
        ),
        pytest.param(
            partial(MultilabelROC, num_labels=3),
            lambda: paddle.rand(100, 3),
            lambda: paddle.randint(low=0, high=2, shape=(100, 3)),
            id="multilabel roc",
        ),
        pytest.param(
            BinaryPrecisionRecallCurve,
