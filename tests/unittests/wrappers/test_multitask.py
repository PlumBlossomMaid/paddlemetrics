...
seed_all(42)
_regression_preds = paddle.rand(NUM_BATCHES, BATCH_SIZE)
_regression_target = paddle.rand(NUM_BATCHES, BATCH_SIZE)
_regression_preds_2 = paddle.rand(NUM_BATCHES, BATCH_SIZE)
_regression_target_2 = paddle.rand(NUM_BATCHES, BATCH_SIZE)
_classification_preds = paddle.randint(low=0, high=2, shape=(NUM_BATCHES, BATCH_SIZE))
_classification_target = paddle.randint(low=0, high=2, shape=(NUM_BATCHES, BATCH_SIZE))
_multitask_preds = {
    "Classification": _classification_preds,
    "Regression": _regression_preds,
}
_multitask_targets = {
    "Classification": _classification_target,
    "Regression": _regression_target,
}
...
def test_error_on_wrong_keys():
    ...
    with pytest.raises(
        ValueError,
        match=re.escape(
            f"Expected arguments `task_preds` and `task_targets` to have the same keys as the wrapped `task_metrics`. Found task_preds.keys() = dict_keys(['Classification']), task_targets.keys() = dict_keys(['Classification', 'Regression']) and self.task_metrics.keys() = {order_dict}dict_keys(['Classification', 'Regression'])"
        ),
    ):
    ...
    with pytest.raises(
        ValueError,
        match=re.escape(
            f"Expected arguments `task_preds` and `task_targets` to have the same keys as the wrapped `task_metrics`. Found task_preds.keys() = dict_keys(['Classification', 'Regression']), task_targets.keys() = dict_keys(['Classification']) and self.task_metrics.keys() = {order_dict}dict_keys(['Classification', 'Regression'])"
        ),
    ):
    ...
    with pytest.raises(
        ValueError,
        match=re.escape(
            f"Expected arguments `task_preds` and `task_targets` to have the same keys as the wrapped `task_metrics`. Found task_preds.keys() = dict_keys(['Classification', 'Regression']), task_targets.keys() = dict_keys(['Classification', 'Regression']) and self.task_metrics.keys() = {order_dict}dict_keys(['Classification'])"
        ),
    ):
...
def test_nested_multitask_wrapper():
    ...
    multitask_preds = {
        "Classification": _classification_preds,
        "Regression": {"Position": _regression_preds, "Size": _regression_preds_2},
    }
    multitask_targets = {
        "Classification": _classification_target,
        "Regression": {"Position": _regression_target, "Size": _regression_target_2},
    }
    classification_metric.update(_classification_preds, _classification_target)
    ...
    regression_results = {
        "Position": regression_position_results,
...
def test_clone_with_prefix_and_postfix():
    ...
    assert set(multitask_metrics.keys()) == {
