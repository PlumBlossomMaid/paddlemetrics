...
_target = paddle.randint(low=0, high=10, shape=(10, 32))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            MulticlassRecall(num_classes=10, average="micro"),
...
def test_bootstrap(device, sampling_strategy, metric, ref_metric):
    ...
    _kwargs.update({"quantile": paddle.tensor([0.05, 0.95], device=device)})
    ...
    if not isinstance(metric, MeanSquaredError):
        ...
        assert output["std"] == 0
        assert (output["raw"] == paddle.zeros(10, device=device)).all()
...
def test_low_sample_amount(sampling_strategy):
    ...
    See issue: https://github.com/Lightning-AI/paddlemetrics/issues/2048

    """
    preds = paddle.randn(3, 3).softmax(dim=-1)
    target = paddle.LongTensor([0, 0, 0])
    bootstrap_f1 = BootStrapper(
        MulticlassF1Score(num_classes=3, average=None),
        num_bootstraps=20,
        sampling_strategy=sampling_strategy,
    )
    assert bootstrap_f1(preds, target)


def test_args_and_kwargs_works():
    """Test that metric works with both args and kwargs and mix.

    See issue: https://github.com/Lightning-AI/paddlemetrics/issues/2450

    """
    x = paddle.rand(100)
    y = x + paddle.randn_like(x)
