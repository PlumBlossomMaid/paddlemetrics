...
def test_raises_error_on_wrong_input():
    ...
    with pytest.raises(
        ValueError,
        match="Expected argument `metric` to be an instance of `paddlemetrics.Metric` but.*",
    ):
        ClasswiseWrapper([])
    with pytest.raises(
        ValueError,
    ...
    with pytest.raises(
        ValueError,
        match="Expected argument `postfix` to either be `None` or a string.*",
    ):
...
def test_using_metriccollection(prefix, postfix):
    ...
    preds = paddle.randn(10, 3).softmax(dim=-1)
    target = paddle.randint(low=0, high=3, shape=(10,))
    val = metric(preds, target)
    assert isinstance(val, dict)
    assert len(val) == 6
...
def test_double_use_of_prefix_with_metriccollection():
    ...
    See issue: https://github.com/Lightning-AI/paddlemetrics/issues/1915

    """
    category_names = ["Tree", "Bush"]
    num_classes = len(category_names)
    input_ = paddle.rand((5, 2, 3, 3))
    target = paddle.ones((5, 2, 3, 3)).long()
