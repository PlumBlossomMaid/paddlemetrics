...
_inputs1 = _Input(
    x=paddle.rand(NUM_BATCHES, BATCH_SIZE, extra_dim),
    y=paddle.rand(NUM_BATCHES, BATCH_SIZE, extra_dim),
)
_inputs2 = _Input(
    x=paddle.rand(NUM_BATCHES, BATCH_SIZE, extra_dim),
    y=paddle.rand(NUM_BATCHES, BATCH_SIZE, extra_dim),
...
def test_precision_case(metric_functional, sk_fn):
    ...
    x = paddle.tensor([[772.0, 112.0], [772.20001, 112.0]])
    res1 = metric_functional(x, zero_diagonal=False)
    res2 = sk_fn(x)
    assert paddle.allclose(x=res1, y=paddle.tensor(res2, dtype=paddle.float32)).item()
