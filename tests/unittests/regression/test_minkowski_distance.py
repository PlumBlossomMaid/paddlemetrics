...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            _reference_scipy_metric_single_target,
        ),
        (
            _multi_target_inputs.preds,
            _multi_target_inputs.target,
            _reference_scipy_metric_multi_target,
        ),
    ],
)
...
def test_error_on_different_shape():
    ...
    with pytest.raises(
        ...
        metric(paddle.randn(50), paddle.randn(100))


def test_error_on_wrong_p_arg():
    """Test that error is raised if wrongly p argument is provided."""
    with pytest.raises(PaddleMetricsUserError, match="Argument ``p`` must be a float.*"):
